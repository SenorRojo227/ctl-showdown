 ...
 const Moves = {
	...
	aromatherapy: {
		...
		onHit(target, source, move) {
			...
			let success = false;
			const allies = [...target.side.pokemon, ..._optionalChain([target, 'access', _ => _.side, 'access', _2 => _2.allySide, 'optionalAccess', _3 => _3.pokemon]) || []];
	...
	auroraveil: {
		...
		condition: {
			...
			durationCallback(target, source, effect) {
				if (_optionalChain([source, 'optionalAccess', _4 => _4.hasItem, 'call', _5 => _5('lightclay')])) {
					return 8;
				}
				return 5;
			},
	...
	burningjealousy: {
		...
		secondary: {
			chance: 100,
			onHit(target, source, move) {
				if (_optionalChain([target, 'optionalAccess', _6 => _6.statsRaisedThisTurn])) {
	...
	endure: {
		...
		condition: {
			...
			onDamage(damage, target, source, effect) {
				if (_optionalChain([effect, 'optionalAccess', _10 => _10.effectType]) === 'Move' && damage >= target.hp) {
	...
	firepledge: {
		...
		onPrepareHit(target, source, move) {
			for (const action of this.queue.list ) {
				if (
					!action.move || !_optionalChain([action, 'access', _11 => _11.pokemon, 'optionalAccess', _12 => _12.isActive]) ||
	...
	focusenergy: {
		...
		condition: {
			onStart(target, source, effect) {
				if (_optionalChain([effect, 'optionalAccess', _13 => _13.id]) === 'zpower') {
	...
	focuspunch: {
		...
		beforeMoveCallback(pokemon) {
			if (_optionalChain([pokemon, 'access', _14 => _14.volatiles, 'access', _15 => _15['focuspunch'], 'optionalAccess', _16 => _16.lostFocus])) {
	...
	followme: {
		...
		condition: {
			duration: 1,
			onStart(target, source, effect) {
				if (_optionalChain([effect, 'optionalAccess', _17 => _17.id]) === 'zpower') {
	...
	gmaxchistrike: {
		...
		condition: {
			...
			onStart(target, source, effect) {
				...
				if (!['imposter', 'psychup', 'transform'].includes(_optionalChain([effect, 'optionalAccess', _18 => _18.id]))) {
			...
			onRestart(target, source, effect) {
				...
				if (!['imposter', 'psychup', 'transform'].includes(_optionalChain([effect, 'optionalAccess', _19 => _19.id]))) {
					this.add('-start', target, 'move: G-Max Chi Strike');
				}
			},
			onModifyCritRatio(critRatio) {
	...
	grasspledge: {
		...
		onPrepareHit(target, source, move) {
			for (const action of this.queue.list ) {
				if (
					!action.move || !_optionalChain([action, 'access', _20 => _20.pokemon, 'optionalAccess', _21 => _21.isActive]) ||
					action.pokemon.fainted || action.maxMove || action.zmove
				) {
					continue;
				}
	...
	grassyterrain: {
		...
		condition: {
			duration: 5,
			durationCallback(source, effect) {
				if (_optionalChain([source, 'optionalAccess', _22 => _22.hasItem, 'call', _23 => _23('terrainextender')])) {
					return 8;
				}
				return 5;
			},
			...
			onFieldStart(field, source, effect) {
				if (_optionalChain([effect, 'optionalAccess', _24 => _24.effectType]) === 'Ability') {
	...
	healbell: {
		...
		onHit(target, source) {
			...
			let success = false;
			const allies = [...target.side.pokemon, ..._optionalChain([target, 'access', _27 => _27.side, 'access', _28 => _28.allySide, 'optionalAccess', _29 => _29.pokemon]) || []];
			for (const ally of allies) {
	...
	healblock: {
		...
		condition: {
			duration: 5,
			durationCallback(target, source, effect) {
				if (_optionalChain([source, 'optionalAccess', _30 => _30.hasAbility, 'call', _31 => _31('persistent')])) {
					this.add('-activate', source, 'ability: Persistent', effect);
					return 7;
				}
				return 5;
			...
			onTryHeal(damage, target, source, effect) {
				if ((_optionalChain([effect, 'optionalAccess', _32 => _32.id]) === 'zpower') || this.effectState.isZ) return damage;
				return false;
			},
	...
	hurricane: {
		...
		flags: {protect: 1, mirror: 1, distance: 1},
		onModifyMove(move, pokemon, target) {
			switch (_optionalChain([target, 'optionalAccess', _33 => _33.effectiveWeather, 'call', _34 => _34()])) {
			case 'raindance':
			case 'primordialsea':
				move.accuracy = true;
				break;
	...
	lightscreen: {
		...
		condition: {
			duration: 5,
			durationCallback(target, source, effect) {
				if (_optionalChain([source, 'optionalAccess', _35 => _35.hasItem, 'call', _36 => _36('lightclay')])) {
					return 8;
				}
				return 5;
			},
	...
	magiccoat: {
		...
		condition: {
			...
			onStart(target, source, effect) {
				...
				if (_optionalChain([effect, 'optionalAccess', _37 => _37.effectType]) === 'Move') {
	...
	mirrormove: {
		...
		onTryHit(target, pokemon) {
			const move = target.lastMove;
			if (!_optionalChain([move, 'optionalAccess', _40 => _40.flags, 'access', _41 => _41['mirror']]) || move.isZ || move.isMax) {
				return false;
			}
	...
	mistyterrain: {
		...
		condition: {
			duration: 5,
			durationCallback(source, effect) {
				if (_optionalChain([source, 'optionalAccess', _42 => _42.hasItem, 'call', _43 => _43('terrainextender')])) {
					return 8;
				}
				return 5;
			},
			...
			onFieldStart(field, source, effect) {
				if (_optionalChain([effect, 'optionalAccess', _44 => _44.effectType]) === 'Ability') {
	...
	psychicterrain: {
		...
		condition: {
			duration: 5,
			durationCallback(source, effect) {
				if (_optionalChain([source, 'optionalAccess', _45 => _45.hasItem, 'call', _46 => _46('terrainextender')])) {
					return 8;
				}
				return 5;
			},
			...
			onFieldStart(field, source, effect) {
				if (_optionalChain([effect, 'optionalAccess', _47 => _47.effectType]) === 'Ability') {
	...
	pursuit: {
		...
		onModifyMove(move, source, target) {
			if (_optionalChain([target, 'optionalAccess', _48 => _48.beingCalledBack]) || _optionalChain([target, 'optionalAccess', _49 => _49.switchFlag])) move.accuracy = true;
		},
		onTryHit(target, pokemon) {
	...
	reflect: {
		...
		condition: {
			duration: 5,
			durationCallback(target, source, effect) {
				if (_optionalChain([source, 'optionalAccess', _50 => _50.hasItem, 'call', _51 => _51('lightclay')])) {
					return 8;
				}
				return 5;
			},
	...
	shelltrap: {
		...
		onTryMove(pokemon) {
			if (!_optionalChain([pokemon, 'access', _54 => _54.volatiles, 'access', _55 => _55['shelltrap'], 'optionalAccess', _56 => _56.gotHit])) {
				this.attrLastMove('[still]');
	...
	spitup: {
		...
		basePower: 0,
		basePowerCallback(pokemon) {
			if (!_optionalChain([pokemon, 'access', _57 => _57.volatiles, 'access', _58 => _58['stockpile'], 'optionalAccess', _59 => _59.layers])) return false;
	...
	suckerpunch: {
		...
		onTry(source, target) {
			const action = this.queue.willMove(target);
			const move = _optionalChain([action, 'optionalAccess', _60 => _60.choice]) === 'move' ? action.move : null;
	...
	thunder: {
		...
		flags: {protect: 1, mirror: 1},
		onModifyMove(move, pokemon, target) {
			switch (_optionalChain([target, 'optionalAccess', _63 => _63.effectiveWeather, 'call', _64 => _64()])) {
			case 'raindance':
			case 'primordialsea':
				move.accuracy = true;
				break;
	...
	torment: {
		...
		condition: {
			...
			onStart(pokemon, source, effect) {
				...
				if (_optionalChain([effect, 'optionalAccess', _65 => _65.id]) === 'gmaxmeltdown') this.effectState.duration = 3;
	...
	trickroom: {
		...
		condition: {
			duration: 5,
			durationCallback(source, effect) {
				if (_optionalChain([source, 'optionalAccess', _66 => _66.hasAbility, 'call', _67 => _67('persistent')])) {
					this.add('-activate', source, 'ability: Persistent', effect);
					return 7;
				}
				return 5;
		...
		condition: {
			...
			onModifyMove(move, pokemon) {
				if (move.secondaries && move.id !== 'secretpower') {
					...
					if (_optionalChain([move, 'access', _68 => _68.self, 'optionalAccess', _69 => _69.chance])) move.self.chance *= 2;
				}
			},
		},
		secondary: null,
	...
	wideguard: {
		...
		condition: {
			...
			onTryHit(target, source, move) {
				...
				if (_optionalChain([move, 'optionalAccess', _70 => _70.target]) !== 'allAdjacent' && move.target !== 'allAdjacentFoes') {
					return;
				}
				if (move.isZ || move.isMax) {
					if (['gmaxoneblow', 'gmaxrapidflow'].includes(move.id)) return;
	...
	wonderroom: {
		...
		condition: {
			duration: 5,
			durationCallback(source, effect) {
				if (_optionalChain([source, 'optionalAccess', _71 => _71.hasAbility, 'call', _72 => _72('persistent')])) {
					this.add('-activate', source, 'ability: Persistent', effect);
					return 7;
				}
				return 5;
