...
function sereneGraceBenefits(move) {
	return _optionalChain([move, 'access', _ => _.secondary, 'optionalAccess', _2 => _2.chance]) && move.secondary.chance >= 20 && move.secondary.chance < 100;
}
 ...
 class RandomTeams {
	 ...
	 */
	 enforceCustomPoolSizeNoComplexBans(
		effectTypeName,
		basicEffectPool,
		requiredCount,
		requiredCountExplanation
	) {
		...
		for (let forme of randomN) {
			...
			if (forme === 'Smeargle') {
				...
				if (species.changesFrom) {
					learnset = this.dex.species.getLearnset(_dex.toID.call(void 0, species.changesFrom));
			...
			for (const species of this.dex.species.all()) {
				...
				if (banReason !== '') {
					...
					if (banReason !== '' || this.dex.species.get(species.baseSpecies).isNonstandard !== species.isNonstandard) {
						const nonexistentCheck = _tags.Tags.nonexistent.genericFilter(species) && nonexistentBanReason;
	...
	randomHCTeam() {
		...
		for (const forme of randomN) {
			...
			if (doItemsExist) {
				...
				item = _optionalChain([itemData, 'optionalAccess', _3 => _3.name]);
			}
			...
			if (doAbilitiesExist) {
				...
				ability = _optionalChain([abilityData, 'optionalAccess', _4 => _4.name]);
			}
	...
	queryMoves(
		...
		if (!_optionalChain([moves, 'optionalAccess', _5 => _5.size])) return counter;
	...
	getLowPriorityItem(
		...
		if (
			...
			(moves.has('boomburst') && Array.from(moves).some(m => _optionalChain([_dex.Dex, 'access', _6 => _6.moves, 'access', _7 => _7.get, 'call', _8 => _8(m), 'access', _9 => _9.boosts, 'optionalAccess', _10 => _10.spe])))
		) return 'Throat Spray';
	...
	randomTeam() {
		...
		while (pokemonPool.length && pokemon.length < this.maxTeamSize) {
			...
			if (this.format.gameType === 'singles') {
				if (!_optionalChain([species, 'access', _11 => _11.randomBattleMoves, 'optionalAccess', _12 => _12.length])) continue;
			} else {
				if (!_optionalChain([species, 'access', _13 => _13.randomDoubleBattleMoves, 'optionalAccess', _14 => _14.length])) continue;
			}
			...
			if (_optionalChain([potd, 'optionalAccess', _15 => _15.exists]) && (pokemon.length === 1 || this.maxTeamSize === 1)) species = potd;
	...
	randomBSSFactoryTeam(side, depth = 0) {
		...
		while (pokemonPool.length && pokemon.length < this.maxTeamSize) {
			...
			for (const typeName of this.dex.types.names()) {
				...
				if (_optionalChain([resistanceAbilities, 'access', _16 => _16[abilityState.id], 'optionalAccess', _17 => _17.includes, 'call', _18 => _18(typeName)]) || !this.dex.getImmunity(typeName, types)) {
