	...
	mefirst: {
		...
		onTryHit(target, pokemon) {
			...
			if (action) {
				...
				// @ts-ignore
				const move = this.dex.getActiveMove(_optionalChain([action, 'access', _ => _.linked, 'optionalAccess', _2 => _2[0]]) || action.move);
	...
	encore: {
		...
		condition: {
			...
			onStart(target) {
				...
				if (!lastMove || target.volatiles['dynamax']) return false;
				if ((lastMove ).isZOrMaxPowered) lastMove = this.dex.moves.get(lastMove.baseMove);
				// @ts-ignore
	...
	iceball: {
		...
		condition: {
			...
			onResidual(target) {
				...
				const lastMove = target.m.lastMoveAbsolute;
				if (_optionalChain([lastMove, 'optionalAccess', _3 => _3.id]) === 'struggle') {
	...
	rollout: {
		...
		condition: {
			...
			onResidual(target) {
				// This is just to ensure the volatile is deleted correctly
				const lastMove = target.m.lastMoveAbsolute;
				if (_optionalChain([lastMove, 'optionalAccess', _4 => _4.id]) === 'struggle') {
