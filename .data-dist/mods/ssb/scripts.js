	...
	actions: {
		...
		hitStepMoveHitLoop(targets, pokemon, move) { // Temporary name
			...
			for (hit = 1; hit <= targetHits; hit++) {
				...
				if (targets.every(target => !_optionalChain([target, 'optionalAccess', _6 => _6.hp]))) break;
				move.hit = hit;
	...
	pokemon: {
		...
		setStatus(status, source, sourceEffect, ignoreImmunities) {
			...
			if (this.status === status.id) {
				if (_optionalChain([(sourceEffect ), 'optionalAccess', _7 => _7.status]) === this.status) {
					this.battle.add('-fail', this, this.status);
				} else if (_optionalChain([(sourceEffect ), 'optionalAccess', _8 => _8.status])) {
			...
			if (!ignoreImmunities && status.id &&
					!((_optionalChain([source, 'optionalAccess', _9 => _9.hasAbility, 'call', _10 => _10('corrosion')]) || _optionalChain([source, 'optionalAccess', _11 => _11.hasAbility, 'call', _12 => _12('hackedcorrosion')]) || _optionalChain([sourceEffect, 'optionalAccess', _13 => _13.id]) === 'cradilychaos') &&
					['tox', 'psn'].includes(status.id))) {
				// the game currently never ignores immunities
				if (!this.runStatusImmunity(status.id === 'tox' ? 'psn' : status.id)) {
					this.battle.debug('immune to status');
					if (_optionalChain([(sourceEffect ), 'optionalAccess', _14 => _14.status])) {
