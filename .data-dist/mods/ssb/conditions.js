...
for (const row of usergroupData) {
	...
	usergroups[_dexdata.toID.call(void 0, cells[0])] = cells[1].trim() || ' ';
}

 function getName(name) {
	const userid = _dexdata.toID.call(void 0, name);
	...
	*/
	...
	stormsurge: {
		...
		durationCallback(target, source, effect) {
			...
			return windSpeeds.indexOf((effect ).basePower) + 2;
		},
	...
	raindance: {
		...
		durationCallback(source) {
			let newDuration = 5;
			let boostNum = 0;
			if (_optionalChain([source, 'optionalAccess', _4 => _4.hasItem, 'call', _5 => _5('damprock')])) {
				newDuration = 8;
			}
			if (_optionalChain([source, 'optionalAccess', _6 => _6.hasAbility, 'call', _7 => _7('kingofatlantis')])) {
