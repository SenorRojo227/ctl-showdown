...
var _sim = require('../.sim-dist');
var _path = require('path');
var _chatjsx = require('./chat-jsx'); var JSX = _chatjsx;
...
// These classes need to be declared here because they aren't hoisted
 class MessageContext {
	...
	extractFormat(formatOrMod) {
		if (!formatOrMod) {
			return {dex: _sim.Dex.includeData(), format: null, isMatch: false};
		}

		const format = _sim.Dex.formats.get(formatOrMod);
		if (format.exists) {
			return {dex: _sim.Dex.forFormat(format), format: format, isMatch: true};
		}

		if (toID(formatOrMod) in _sim.Dex.dexes) {
			return {dex: _sim.Dex.mod(toID(formatOrMod)).includeData(), format: null, isMatch: true};
		}
 ...
 */
 class CommandContext extends MessageContext {
	...
	checkBroadcast(overrideCooldown, suppressMessage) {
		...
		if (this.user.locked && !(_optionalChain([this, 'access', _48 => _48.room, 'optionalAccess', _49 => _49.roomid, 'access', _50 => _50.startsWith, 'call', _51 => _51('help-')]) || _optionalChain([this, 'access', _52 => _52.pmTarget, 'optionalAccess', _53 => _53.can, 'call', _54 => _54('lock')]))) {
	...
	checkChat(message = null, room = null, targetUser = null) {
		...
		if (Config.restrictLinks && !user.autoconfirmed) {
			...
			const allLinksWhitelisted = !links || links.every(link => {
				...
				const domain = _optionalChain([domainMatches, 'optionalAccess', _55 => _55[1]]);
				const hostMatches = /^(?:http:\/\/|https:\/\/)?([^/]*[^/.])\.?($|\/|:)/.exec(link);
				let host = _optionalChain([hostMatches, 'optionalAccess', _56 => _56[1]]);
				if (_optionalChain([host, 'optionalAccess', _57 => _57.startsWith, 'call', _58 => _58('www.')])) host = host.slice(4);
				if (!domain || !host) return null;
				return LINK_WHITELIST.includes(host) || LINK_WHITELIST.includes(`*.${domain}`);
			});
			if (!allLinksWhitelisted && !(_optionalChain([targetUser, 'optionalAccess', _59 => _59.can, 'call', _60 => _60('lock')]) || _optionalChain([room, 'optionalAccess', _61 => _61.settings, 'access', _62 => _62.isHelp]))) {
		...
		if (_optionalChain([room, 'optionalAccess', _63 => _63.settings, 'access', _64 => _64.highTraffic]) &&
			...
			for (const tag of tags) {
				...
				if (tagName === 'button') {
					if ((!this.room || this.room.settings.isPersonal || this.room.settings.isPrivate === true) && !this.user.can('lock')) {
						const buttonName = _optionalChain([/ name ?= ?"([^"]*)"/i, 'access', _65 => _65.exec, 'call', _66 => _66(tagContent), 'optionalAccess', _67 => _67[1]]);
						const buttonValue = _optionalChain([/ value ?= ?"([^"]*)"/i, 'access', _68 => _68.exec, 'call', _69 => _69(tagContent), 'optionalAccess', _70 => _70[1]]);
	...
	refreshPage(pageid) {
		if (_optionalChain([this, 'access', _71 => _71.connection, 'access', _72 => _72.openPages, 'optionalAccess', _73 => _73.has, 'call', _74 => _74(pageid)])) {
			this.parse(`/join view-${pageid}`);
		}
	}
	closePage(pageid) {
		for (const connection of this.user.connections) {
			if (_optionalChain([connection, 'access', _75 => _75.openPages, 'optionalAccess', _76 => _76.has, 'call', _77 => _77(pageid)])) {
 ...
 const Chat = new (_class = class {
	 ...
	 *********************************************************/
	 ...
	 __init6() {this.destroyHandlers = [Artemis.destroy]}
	...
	async loadTranslations() {
		...
		for (const dirname of directories) {
			...
			const languageID = _sim.Dex.toID(dirname);
	...
	tr(language, strings = '', ...keys) {
		...
		const entry = _optionalChain([exports.Chat, 'access', _80 => _80.translations, 'access', _81 => _81.get, 'call', _82 => _82(language), 'optionalAccess', _83 => _83.get, 'call', _84 => _84(trString)]);
	 ...
	 */
	__init21() {this.database = _lib.SQL.call(void 0, module, {
		file: ('Config' in global && Config.nofswriting) ? ':memory:' : PLUGIN_DATABASE_PATH,
		processes: _optionalChain([global, 'access', _85 => _85.Config, 'optionalAccess', _86 => _86.chatdbprocesses]) || 1,
	})}
	__init22() {this.databaseReadyPromise = null}

	async prepareDatabase() {
		if (!_friends.PM.isParentProcess) return; // We don't need a database in a subprocess that requires Chat.
		...
		for (const migrationFile of (await _lib.FS.call(void 0, migrationsFolder).readdir())) {
			const migrationVersion = parseInt(_optionalChain([/v(\d+)\.sql$/, 'access', _87 => _87.exec, 'call', _88 => _88(migrationFile), 'optionalAccess', _89 => _89[1]]) || '');
		...
		_lib.Utils.sortBy(migrationsToRun, ({version}) => version);
		for (const {file} of migrationsToRun) {
			await this.database.runFile(_path.resolve.call(void 0, migrationsFolder, file));
		}

		exports.Chat.destroyHandlers.push(() => _optionalChain([exports.Chat, 'access', _90 => _90.database, 'optionalAccess', _91 => _91.destroy, 'call', _92 => _92()]));
	}

	 __init23() {this.MessageContext = MessageContext}
	 __init24() {this.CommandContext = exports.CommandContext = CommandContext}
	 __init25() {this.PageContext = exports.PageContext = PageContext}
	 __init26() {this.ErrorMessage = exports.ErrorMessage = ErrorMessage}
	 __init27() {this.Interruption = exports.Interruption = Interruption}

	// JSX handling
	 __init28() {this.JSX = JSX}
	 __init29() {this.html = JSX.html}
	 __init30() {this.h = JSX.h}
	 __init31() {this.Fragment = JSX.Fragment}
	...
	parse(message, room, user, connection) {
		...
		const initialRoomlogLength = _optionalChain([room, 'optionalAccess', _93 => _93.log, 'access', _94 => _94.getLineCount, 'call', _95 => _95()]);
		const context = new CommandContext({message, room, user, connection});
		const start = Date.now();
		const result = context.parse();
		if (typeof _optionalChain([result, 'optionalAccess', _96 => _96.then]) === 'function') {
			void result.then(() => {
	...
	logSlowMessage(start, context) {
		...
		const logMessage = (
			...
			`<${context.room ? context.room.roomid : context.pmTarget ? `PM:${_optionalChain([context, 'access', _97 => _97.pmTarget, 'optionalAccess', _98 => _98.name])}` : 'CMD'}> ` +
	...
	loadPluginDirectory(dir, depth = 0) {
		for (const file of _lib.FS.call(void 0, dir).readdirSync()) {
			const path = _path.resolve.call(void 0, dir, file);
			if (_lib.FS.call(void 0, path).isDirectorySync()) {
				depth++;
	...
	annotateCommands(commandTable, namespace = '') {
		for (const cmd in commandTable) {
			...
			entry.requiresRoom = _optionalChain([/requireRoom\((?:'|"|`)(.*?)(?:'|"|`)/, 'access', _99 => _99.exec, 'call', _100 => _100(handlerCode), 'optionalAccess', _101 => _101[1]])  || /this\.requireRoom\(/.test(handlerCode);
			...
			entry.requiredPermission = _optionalChain([/this\.(?:checkCan|privately(?:Check)?Can)\(['`"]([a-zA-Z0-9]+)['"`](\)|, )/, 'access', _102 => _102.exec, 'call', _103 => _103(handlerCode), 'optionalAccess', _104 => _104[1]]);
	...
	loadPlugin(plugin, name) {
		...
		if (plugin.onRenameRoom) {
			if (!exports.Chat.handlers['onRenameRoom']) exports.Chat.handlers['onRenameRoom'] = [];
			exports.Chat.handlers['onRenameRoom'].push(plugin.onRenameRoom);
		}
		if (plugin.onRoomClose) {
			if (!exports.Chat.handlers['onRoomClose']) exports.Chat.handlers['onRoomClose'] = [];
...

 {
	...
	toDurationString(val, options = {}) {
		...
		let precision = (_optionalChain([options, 'optionalAccess', _105 => _105.precision]) ? options.precision : 3);
		if (_optionalChain([options, 'optionalAccess', _106 => _106.hhmmss])) {
	...
	getDataPokemonHTML(species, gen = 8, tier = '') {
		...
		if (gen >= 3) {
			...
			if (species.abilities['1'] && (gen >= 4 || _sim.Dex.abilities.get(species.abilities['1']).gen === 3)) {
	...
	parseArguments(str, delim = ',', paramDelim = '=', useIDs = true) {
		const result = {};
		for (const part of str.split(delim)) {
			let [key, val] = _lib.Utils.splitFirst(part, paramDelim).map(f => f.trim());
			if (useIDs) key = toID(key);
			if (!toID(key) || !toID(val)) {
				throw new exports.Chat.ErrorMessage(`Invalid option ${part}. Must be in [key]${paramDelim}[value] format.`);
			}
	...
	refreshPageFor(
		...
		for (const id in room.users) {
			if (_optionalChain([ignoreUsers, 'optionalAccess', _107 => _107.includes, 'call', _108 => _108(id )])) continue;
	...
	maybeNotifyBlocked(blocked, targetUser, user) {
	 ...
	 __init33() {this.formatText = _chatformatter.formatText}
	 __init34() {this.linkRegex = _chatformatter.linkRegex}
	 __init35() {this.stripFormatting = _chatformatter.stripFormatting}

	 __init36() {this.filterWords = {}}
	 __init37() {this.monitors = {}}
...
(CommandContext.prototype ).targetUserOrSelf = function ( target, exactName) {
	...
	this.targetUsername = _optionalChain([user, 'optionalAccess', _109 => _109.name]) || target;
	return user;
};
...
if (!process.send) {
	exports.Chat.database.spawn(Config.chatdbprocesses || 1);
	...
	_lib.Repl.start('chat-db', cmd => eval(cmd));
}
