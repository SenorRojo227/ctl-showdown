...
async function updateserver(context, codePath) {
 ...
 const commands = {
	...
	async evalsql(target, room) {
		...
		const database = _lib.SQL.call(void 0, module, {
		...
		try {
			...
			if ((result ).err) parseError(result );
		} catch (err) {
			// it's not getting data, but it might still be a valid statement - try to run instead
			if (_optionalChain([err, 'access', _19 => _19.stack, 'optionalAccess', _20 => _20.includes, 'call', _21 => _21(`Use run() instead`)])) {
				try {
		...
		_optionalChain([logRoom, 'optionalAccess', _22 => _22.roomlog, 'call', _23 => _23(formattedResult)]);
