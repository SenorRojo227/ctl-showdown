...
const OFFLINE_SYMBOL = ` \u25CC `;
 ...
 const commands = {
	...
	async show(target, room, user, connection) {
		if (!_optionalChain([room, 'optionalAccess', _109 => _109.persist]) && !this.pmTarget && !_optionalChain([room, 'optionalAccess', _110 => _110.roomid, 'access', _111 => _111.startsWith, 'call', _112 => _112('help-')])) {
		...
		const [link, comment] = _lib.Utils.splitFirst(target, ',').map(f => f.trim());
		...
		if (_youtube.YouTube.linkRegex.test(link)) {
			...
			const channelId = _optionalChain([_youtube.Twitch, 'access', _113 => _113.linkRegex, 'access', _114 => _114.exec, 'call', _115 => _115(link), 'optionalAccess', _116 => _116[2], 'optionalAccess', _117 => _117.trim, 'call', _118 => _118()]);
		...
		if (comment) {
			...
			buf += _lib.Utils.html`<br />(${comment})</div>`;
		}
	...
	code(target, room, user, connection) {
		...
		if (_optionalChain([this, 'access', _119 => _119.room, 'optionalAccess', _120 => _120.settings, 'access', _121 => _121.isPersonal]) !== false && this.shouldBroadcast()) {
