 ...
 */
 function runPromote(
	...
	if (currentSymbol === symbol) {
		throw new Chat.ErrorMessage(`User '${username}' is already a ${_optionalChain([nextGroup, 'optionalAccess', _ => _.name]) || symbol || 'regular user'} in this room.`);
	}
	...
	if (_optionalChain([targetUser, 'optionalAccess', _2 => _2.locked]) && room.persist && room.settings.isPrivate !== true && nextGroup.rank > 2) {
 ...
 const commands = {
	roomowner(target, room, user) {
		...
		if (room.settings.isPrivate === true) {
			...
			_optionalChain([Rooms, 'access', _3 => _3.get, 'call', _4 => _4(`upperstaff`), 'optionalAccess', _5 => _5.addByUser, 'call', _6 => _6(user, `<<${room.roomid}>> ${message}`), 'access', _7 => _7.update, 'call', _8 => _8()]);
		} else {
	...
	roompromote(target, room, user, connection, cmd) {
		...
		for (const toPromote of users) {
			...
			try {
				...
				if (_optionalChain([err, 'access', _9 => _9.name, 'optionalAccess', _10 => _10.endsWith, 'call', _11 => _11('ErrorMessage')])) {
			...
			const targetUser = Users.getExact(userid);
			const name = _optionalChain([targetUser, 'optionalAccess', _12 => _12.name]) || toPromote;
			...
			if (this.pmTarget && targetUser) {
				...
				_optionalChain([shouldPopup, 'optionalAccess', _13 => _13.popup, 'call', _14 => _14(`You were demoted to Room ${nextGroupName} by ${user.name} in ${room.roomid}.`)]);
			} else if (nextSymbol === '#') {
				...
				_optionalChain([logRoom, 'optionalAccess', _15 => _15.addByUser, 'call', _16 => _16(user, `<<${room.roomid}>> ${name} was appointed Room Owner by ${user.name}`)]);
				this.modlog('ROOM OWNER', userid);
				_optionalChain([shouldPopup, 'optionalAccess', _17 => _17.popup, 'call', _18 => _18(`You were promoted to ${nextGroupName} by ${user.name} in ${room.roomid}.`)]);
			} else {
				this.addModAction(`${name} was promoted to Room ${nextGroupName} by ${user.name}.`);
				this.modlog(`ROOM${nextGroupName.toUpperCase()}`, userid);
				_optionalChain([shouldPopup, 'optionalAccess', _19 => _19.popup, 'call', _20 => _20(`You were promoted to Room ${nextGroupName} by ${user.name} in ${room.roomid}.`)]);
			}
	...
	authority(target, room, user, connection) {
		if (target && target !== '+') {
			...
			const availableRoom = _optionalChain([targetRoom, 'optionalAccess', _21 => _21.checkModjoin, 'call', _22 => _22(user)]);
	...
	roomauth(target, room, user, connection, cmd) {
		...
		if (!_optionalChain([targetRoom, 'optionalAccess', _23 => _23.checkModjoin, 'call', _24 => _24(user)])) {
		...
		const buffer = _lib.Utils.sortBy(
			...
			return `${group}:\n` +
				_lib.Utils.sortBy(names).map(userid => {
					const isOnline = _optionalChain([Users, 'access', _25 => _25.get, 'call', _26 => _26(userid), 'optionalAccess', _27 => _27.statusType]) === 'online';
	...
	userauth(target, room, user, connection) {
		...
		const targetUsername = _optionalChain([targetUser, 'optionalAccess', _28 => _28.name]) || target;
	...
	part(target, room, user, connection) {
		...
		if (!targetRoom) {
			if (target.startsWith('view-')) {
				_optionalChain([connection, 'access', _29 => _29.openPages, 'optionalAccess', _30 => _30.delete, 'call', _31 => _31(target.slice(5))]);
				if (!_optionalChain([connection, 'access', _32 => _32.openPages, 'optionalAccess', _33 => _33.size])) connection.openPages = null;
	...
	warn(target, room, user) {
		...
		this.checkChat();
		if (_optionalChain([room, 'optionalAccess', _34 => _34.settings, 'access', _35 => _35.isPersonal]) && !user.can('warn' )) {
		...
		const saveReplay = globalWarn && _optionalChain([room, 'optionalAccess', _36 => _36.battle]);
	...
	redir(target, room, user, connection) {
		...
		if (!_optionalChain([targetUser, 'optionalAccess', _38 => _38.connected])) {
	...
	unmute(target, room, user) {
		...
		const successfullyUnmuted = room.unmute(
			_optionalChain([targetUser, 'optionalAccess', _39 => _39.id]) || toID(targetUsername), `Your mute in '${room.title}' has been lifted.`
		);
	...
	async lock(target, room, user, connection, cmd) {
		...
		// Automatically upload replays as evidence/reference to the punishment
		if (_optionalChain([room, 'optionalAccess', _40 => _40.battle])) this.parse('/savereplay forpunishment');
		return true;
	},
	lockhelp: [
	...
	unlock(target, room, user) {
		...
		if (_optionalChain([targetUser, 'optionalAccess', _41 => _41.namelocked])) {
			return this.errorReply(`User ${targetUser.name} is namelocked, not locked. Use /unnamelock to unnamelock them.`);
		}
		let reason = '';
		if (_optionalChain([targetUser, 'optionalAccess', _42 => _42.locked]) && targetUser.locked.startsWith('#')) {
	...
	unlockname(target, room, user) {
		...
		if (punishment.id === userid || _optionalChain([Users, 'access', _43 => _43.get, 'call', _44 => _44(userid), 'optionalAccess', _45 => _45.previousIDs, 'access', _46 => _46.includes, 'call', _47 => _47(punishment.id )])) {
	...
	unlockip(target, room, user) {
		...
		for (const curUser of Users.findUsers([], [target])) {
			if (
				(range ? curUser.locked === '#rangelock' : !_optionalChain([curUser, 'access', _48 => _48.locked, 'optionalAccess', _49 => _49.startsWith, 'call', _50 => _50('#')])) &&
	...
	async globalban(target, room, user, connection, cmd) {
		...
		if (Users.isTrusted(userid)) {
			if (force) {
				const from = runCrisisDemote(userid);
				Monitor.log(`[CrisisMonitor] ${name} was globally banned by ${user.name} and demoted from ${_optionalChain([from, 'optionalAccess', _51 => _51.join, 'call', _52 => _52(", ")])}.`);
				this.globalModlog("CRISISDEMOTE", targetUser, ` from ${_optionalChain([from, 'optionalAccess', _53 => _53.join, 'call', _54 => _54(", ")])}`);
			} else {
		...
		const {privateReason, publicReason} = this.parseSpoiler(reason);
		_optionalChain([targetUser, 'optionalAccess', _55 => _55.popup, 'call', _56 => _56(
		...
		_optionalChain([room, 'optionalAccess', _57 => _57.hideText, 'call', _58 => _58([
			...affected.map(u => u.id),
			toID(inputUsername),
		])]);
	...
	banip(target, room, user) {
		...
		if (_optionalChain([curPunishment, 'optionalAccess', _59 => _59.type]) === 'BAN') {
	...
	yearlockname(target, room, user) {
		...
		if (_optionalChain([targetUser, 'optionalAccess', _60 => _60.registered]) && !force) {
	...
	modnote(target, room, user, connection) {
		...
		const targetUserid = toID(_optionalChain([/\[([^\]]+)\]/, 'access', _61 => _61.exec, 'call', _62 => _62(target), 'optionalAccess', _63 => _63[1]])) || null;
		if (
	...
	promote(target, room, user, connection, cmd) {
		...
		const userid = toID(targetUsername);
		const name = _optionalChain([targetUser, 'optionalAccess', _64 => _64.name]) || targetUsername;

		if (!userid) return this.parse('/help promote');

		const currentGroup = _optionalChain([targetUser, 'optionalAccess', _65 => _65.tempGroup]) || Users.globalAuth.get(userid);
	...
	trustuser(target, room, user, connection, cmd) {
		...
		if (rest) return this.errorReply(`This command does not support specifying a reason.`);
		const userid = toID(targetUsername);
		const name = _optionalChain([targetUser, 'optionalAccess', _66 => _66.name]) || targetUsername;
	...
	sectionleader(target, room, user, connection, cmd) {
		...
		if (Users.globalAuth.sectionLeaders.has(_optionalChain([targetUser, 'optionalAccess', _67 => _67.id]) || userid) && !demoting) {
			throw new Chat.ErrorMessage(`${name} is already a Section Leader of ${_roomsettings.RoomSections.sectionNames[section]}.`);
		} else if (!Users.globalAuth.sectionLeaders.has(_optionalChain([targetUser, 'optionalAccess', _68 => _68.id]) || userid) && demoting) {
		...
		if (!demoting) {
			...
			_optionalChain([targetUser, 'optionalAccess', _69 => _69.popup, 'call', _70 => _70(`You were appointed Section Leader of ${_roomsettings.RoomSections.sectionNames[section]} by ${user.name}.`)]);
		} else {
			...
			if (_optionalChain([staffRoom, 'optionalAccess', _71 => _71.auth, 'access', _72 => _72.getDirect, 'call', _73 => _73(userid)])  === '\u25B8') this.parse(`/msgroom staff,/roomdeauth ${userid}`);
			_optionalChain([targetUser, 'optionalAccess', _74 => _74.popup, 'call', _75 => _75(`You were demoted from Section Leader of ${_roomsettings.RoomSections.sectionNames[section]} by ${user.name}.`)]);
		}
	...
	htmldeclare(target, room, user) {
		...
		for (const u in room.users) {
			_optionalChain([Users, 'access', _76 => _76.get, 'call', _77 => _77(u), 'optionalAccess', _78 => _78.sendTo, 'call', _79 => _79(
				room,
	...
	notifyuser(target, room, user, connection, cmd) {
		...
		if (!_optionalChain([targetUser, 'optionalAccess', _80 => _80.connected])) return this.errorReply(`User '${targetUsername}' not found.`);
	...
	forcerename(target, room, user) {
		...
		const targetID = _optionalChain([targetUser, 'optionalAccess', _81 => _81.id]) || toID(targetUsername);
		...
		if (_optionalChain([targetUser, 'optionalAccess', _82 => _82.connected])) {
		...
		if (_optionalChain([room, 'optionalAccess', _83 => _83.roomid]) !== 'staff') {
			if (_optionalChain([room, 'optionalAccess', _84 => _84.roomid, 'access', _85 => _85.startsWith, 'call', _86 => _86('help-')])) {
				this.addModAction(`${_optionalChain([targetUser, 'optionalAccess', _87 => _87.name]) || targetID} ${forceRenameMessage}`);
			} else {
				this.privateModAction(`${_optionalChain([targetUser, 'optionalAccess', _88 => _88.name]) || targetID} ${forceRenameMessage}`);
			}
		}
		const roomMessage = this.pmTarget ? `<PM:${this.pmTarget.id}>` :
			room && room.roomid !== 'staff' ? `«<a href="/${room.roomid}" target="_blank">${room.roomid}</a>» ` :
			'';
		const rankMessage = _optionalChain([targetUser, 'optionalAccess', _89 => _89.getAccountStatusString, 'call', _90 => _90()]) || "";
		Rooms.global.notifyRooms(
			['staff'],
			`|html|${roomMessage}` + _lib.Utils.html`<span class="username">${_optionalChain([targetUser, 'optionalAccess', _91 => _91.name]) || targetID}</span> ${rankMessage} ${forceRenameMessage}`
		);

		_optionalChain([targetUser, 'optionalAccess', _92 => _92.resetName, 'call', _93 => _93(true)]);
		return true;
	},
	forcerenamehelp: [
	...
	noforcerename: {
		add(target, room, user) {
			const [targetUsername, rest] = _lib.Utils.splitFirst(target, ',').map(f => f.trim());
			const targetId = toID(targetUsername);
	...
	async namelock(target, room, user, connection, cmd) {
		...
		if (_optionalChain([targetUser, 'optionalAccess', _94 => _94.namelocked]) && !week) {
		...
		this.privateGlobalModAction(`${_optionalChain([targetUser, 'optionalAccess', _95 => _95.name]) || userid} was ${week ? 'week' : ''}namelocked by ${user.name}${reasonText}`);
		...
		// Automatically upload replays as evidence/reference to the punishment
		if (_optionalChain([room, 'optionalAccess', _96 => _96.battle])) this.parse('/savereplay forpunishment');
	...
	unnamelock(target, room, user) {
		...
		const targetUser = Users.get(target);
		let reason = '';
		if (_optionalChain([targetUser, 'optionalAccess', _97 => _97.namelocked])) {
	...
	async groupchatban(target, room, user, connection, cmd) {
		...
		for (const roomid of createdGroupchats) {
			...
			const participants = _optionalChain([targetRoom, 'access', _98 => _98.warnParticipants, 'optionalCall', _99 => _99(
