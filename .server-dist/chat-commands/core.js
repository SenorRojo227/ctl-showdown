 ...
 const crqHandlers = {
	userdetails(target, user, trustable) {
		...
		return {
			...
			rooms: roomList,
			friended: _optionalChain([user, 'access', _7 => _7.friends, 'optionalAccess', _8 => _8.has, 'call', _9 => _9(targetUser.id)]) || undefined,
		};
	},
 ...
 const commands = {
	...
	userlist(target, room, user) {
		...
		for (const id in room.users) {
			...
			userList.push(_lib.Utils.escapeHTML(curUser.getIdentity(room)));
		}
	...
	hidereplay(target, room, user, connection) {
		if (!_optionalChain([room, 'optionalAccess', _12 => _12.battle])) return this.errorReply(`Must be used in a battle.`);
	...
	timer(target, room, user) {
		target = toID(target);
		room = this.requireRoom();
		if (!_optionalChain([room, 'access', _15 => _15.game, 'optionalAccess', _16 => _16.timer])) {
	...
	challenge(target, room, user, connection) {
		...
		if (!_optionalChain([targetUser, 'optionalAccess', _17 => _17.connected])) {
	...
	help(target, room, user) {
		...
		const getHelp = (namespace, cmds) => {
			...
			const requiredPerm = _optionalChain([curHandler, 'optionalAccess', _18 => _18.requiredPermission]) || 'lock';
			if (_optionalChain([curHandler, 'optionalAccess', _19 => _19.isPrivate]) && !user.can(requiredPerm )) {
