...
function removeNohost(hostname) {
 ...
 const IPTools = new (_class = class {constructor() { _class.prototype.__init.call(this);_class.prototype.__init2.call(this);_class.prototype.__init3.call(this);_class.prototype.__init4.call(this);_class.prototype.__init5.call(this);_class.prototype.__init6.call(this);_class.prototype.__init7.call(this);_class.prototype.__init8.call(this);_class.prototype.__init9.call(this);_class.prototype.__init10.call(this);_class.prototype.__init11.call(this); }
	 ...
	 __init3() {this.ipRegex = /^(25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9]?[0-9])\.(25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9]?[0-9])\.(25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9]?[0-9])\.(25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9]?[0-9])$/}
	...
	ipToNumber(ip) {
		...
		for (const part of parts) {
			...
			const partAsInt = _lib.Utils.parseExactInt(part);
	...
	getCidrRange(cidr) {
		...
		const bits = _lib.Utils.parseExactInt(cidr.slice(index + 1));
	...
	stringToRange(range) {
		...
		if (index <= 0) {
			if (range.includes('/')) return exports.IPTools.getCidrRange(range);
	...
	checkRangeConflicts(insertion, sortedRanges, widen) {
		if (insertion.maxIP < insertion.minIP) {
			throw new Error(
				`Invalid data for address range ${exports.IPTools.rangeToString(insertion)} (${insertion.host})`
			);
		}
		...
		if (iMin < sortedRanges.length) {
			...
			if (insertion.minIP <= next.minIP && insertion.maxIP >= next.maxIP) {
				...
				throw new Error(
					`Too wide: ${exports.IPTools.rangeToString(insertion)} (${insertion.host})\n` +
					`Intersects with: ${exports.IPTools.rangeToString(next)} (${next.host})`
				);
			}
			if (insertion.maxIP >= next.minIP) {
				throw new Error(
					`Could not insert: ${exports.IPTools.rangeToString(insertion)} ${insertion.host}\n` +
					`Intersects with: ${exports.IPTools.rangeToString(next)} (${next.host})`
				);
			}
		}
		if (iMin > 0) {
			...
			if (insertion.minIP >= prev.minIP && insertion.maxIP <= prev.maxIP) {
				throw new Error(
					`Too narrow: ${exports.IPTools.rangeToString(insertion)} (${insertion.host})\n` +
	...
	sortRanges() {
		_lib.Utils.sortBy(exports.IPTools.ranges, range => range.minIP);
	}
