...
const TIMER_COOLDOWN = 20 * SECONDS;
 ...
 class RoomBattleTimer {
	...
	start(requester) {
		...
		if (this.battle.ended) {
			_optionalChain([requester, 'optionalAccess', _2 => _2.sendTo, 'call', _3 => _3(this.battle.roomid, `|inactiveoff|The timer can't be enabled after a battle has ended.`)]);
			return false;
		}
		if (this.timer) {
	...
	checkActivity() {
		if (this.battle.ended) return;
		for (const player of this.battle.players) {
			const isConnected = !!_optionalChain([player, 'optionalAccess', _4 => _4.active]);
 ...
 class RoomBattle extends RoomGames.RoomGame {
	...
	constructor(room, options) {
		...
		this.addPlayer(_optionalChain([options, 'access', _5 => _5.p1, 'optionalAccess', _6 => _6.user]) || null, options.p1);
		this.addPlayer(_optionalChain([options, 'access', _7 => _7.p2, 'optionalAccess', _8 => _8.user]) || null, options.p2);
		if (this.playerCap > 2) {
			this.addPlayer(_optionalChain([options, 'access', _9 => _9.p3, 'optionalAccess', _10 => _10.user]) || null, options.p3);
			this.addPlayer(_optionalChain([options, 'access', _11 => _11.p4, 'optionalAccess', _12 => _12.user]) || null, options.p4);
		}
	...
	checkActive() {
		...
		if (this.ended || !this.started) {
			active = false;
		} else if (!_optionalChain([this, 'access', _13 => _13.p1, 'optionalAccess', _14 => _14.active])) {
			active = false;
		} else if (!_optionalChain([this, 'access', _15 => _15.p2, 'optionalAccess', _16 => _16.active])) {
			active = false;
		} else if (this.playerCap > 2) {
			if (!_optionalChain([this, 'access', _17 => _17.p3, 'optionalAccess', _18 => _18.active])) {
				active = false;
			} else if (!_optionalChain([this, 'access', _19 => _19.p4, 'optionalAccess', _20 => _20.active])) {
				active = false;
			}
		}
	...
	async onEnd(winner) {
		...
		Chat.runHandlers('onBattleEnd', this, winnerid, [p1id, p2id, _optionalChain([this, 'access', _21 => _21.p3, 'optionalAccess', _22 => _22.id]), _optionalChain([this, 'access', _23 => _23.p4, 'optionalAccess', _24 => _24.id])].filter(Boolean));
		if (this.room.rated) {
			this.room.rated = 0;
		...
		if (this.replaySaved || Config.autosavereplays) {
			...
			if (_optionalChain([uploader, 'optionalAccess', _25 => _25.connections, 'access', _26 => _26[0]])) {
		...
		if (_optionalChain([parentGame, 'optionalAccess', _27 => _27.onBattleWin])) {
			// @ts-ignore
	...
	onLeave(user, oldUserid) {
		...
		if (_optionalChain([player, 'optionalAccess', _28 => _28.active])) {
	...
	addPlayer(user, playerOpts) {
		...
		if (_optionalChain([user, 'optionalAccess', _29 => _29.inRooms, 'access', _30 => _30.has, 'call', _31 => _31(this.roomid)])) this.onConnect(user);
		return player;
	}
	...
	checkPrivacySettings(options) {
		...
		if (privacySetter.size) {
			...
			} else if (!options.tour || (_optionalChain([room, 'access', _32 => _32.tour, 'optionalAccess', _33 => _33.allowModjoin]))) {
	...
	checkForcedUserSettings(user) {
		...
		if (
			this.players.some(p => _optionalChain([p, 'access', _34 => _34.getUser, 'call', _35 => _35(), 'optionalAccess', _36 => _36.battleSettings, 'access', _37 => _37.special])) ||
	...
	static battleForcedSetting(user, key) {
		if (Config.forcedpublicprefixes) {
			for (const prefix of Config.forcedpublicprefixes) {
				_optionalChain([Chat, 'access', _38 => _38.plugins, 'access', _39 => _39['username-prefixes'], 'optionalAccess', _40 => _40.prefixManager, 'access', _41 => _41.addPrefix, 'call', _42 => _42(prefix, 'privacy')]);
			}
	...
	updatePlayer(player, user, playerOpts) {
		...
		if (user) {
			const options = {
				name: player.name,
				avatar: user.avatar,
				team: _optionalChain([playerOpts, 'optionalAccess', _43 => _43.team]),
			};
			void this.stream.write(`>player ${slot} ` + JSON.stringify(options));
	...
	start() {
		...
		const suspectTest = _optionalChain([Chat, 'access', _44 => _44.plugins, 'access', _45 => _45['suspect-tests'], 'optionalAccess', _46 => _46.suspectTests, 'access', _47 => _47[this.format]]);
		if (suspectTest) {
		...
		try {
			...
			if (battle) {
				for (const side of battle.sides) {
					if (_optionalChain([side, 'optionalAccess', _48 => _48.requestState])) {
