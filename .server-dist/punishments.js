 ...
 *********************************************************/
 ...
 const Punishments = new (_class = class {
	 ...
	 */
	 __init10() {this.bannedGroupchatParticipants = {}}
	/** roomid:timestamp map */
	 __init11() {this.lastGroupchatMonitorTime = {}}
	/**
	 * Map<userid that has been warned, reason they were warned for>
	 */
	 __init12() {this.offlineWarns = new Map()}
	/**
	 ...
	 * Keys in the map correspond to PunishInfo */
	 __init13() {this.punishmentTypes = new Map([
	 ...
	 *
	 */
	 __init14() {this.roomPunishmentTypes = new Map([
	...
	async lock(
		...
		const punishment = {type: 'LOCK', id, expireTime, reason: reason, rest} ;
	...
	isBattleBanned(user) {
		...
		for (const ip of user.ips) {
			...
			if (punishment) {
				if (exports.Punishments.isSharedIp(ip) && user.autoconfirmed) return;
				return punishment;
			}
		}
	}
	...
	isGroupchatBanned(user) {
		...
		if (targetUser && !targetUser.trusted) {
			for (const ip of targetUser.ips) {
				...
				if (punishment) {
					if (exports.Punishments.isSharedIp(ip) && targetUser.autoconfirmed) return;
					return punishment;
				}
			}
		}
	...
	isTicketBanned(user) {
		...
		if (ips.some(ip => exports.Punishments.isSharedIp(ip))) return false;
	...
	__init15() {this.sortedTypes = ['TICKETBAN', 'LOCK', 'NAMELOCK', 'BAN']}
	__init16() {this.sortedRoomTypes = [...(_optionalChain([global, 'access', _56 => _56.Punishments, 'optionalAccess', _57 => _57.sortedRoomTypes]) || []), 'ROOMBAN', 'BLACKLIST']}
	...
	__init17() {this.interactions = {
		NAMELOCK: {overrides: ['LOCK']},
	}}

	__init18() {this.roomInteractions = {
	...
	checkName(user, userid, registered) {
		...
		if (user.namelocked) {
			let punishment = _optionalChain([exports.Punishments, 'access', _58 => _58.userids, 'access', _59 => _59.get, 'call', _60 => _60(user.namelocked), 'optionalAccess', _61 => _61[0]]);
			if (!punishment) punishment = {type: 'NAMELOCK', id: user.namelocked, expireTime: 0, reason: ''};
			punishments.push(punishment);
		}
		if (user.locked) {
			let punishment = _optionalChain([exports.Punishments, 'access', _62 => _62.userids, 'access', _63 => _63.get, 'call', _64 => _64(user.locked), 'optionalAccess', _65 => _65[0]]);
		...
		for (const punishment of punishments) {
				...
				if (punishUserid !== user.id && exports.Punishments.isSharedIp(user.latestIp) && user.autoconfirmed) {
					...
					if (!exports.Punishments.userids.getByType(userid, 'BATTLEBAN')) {
						...
						user.send(
							...
							`${punishment.reason ? _lib.Utils.html `\n\nReason: ${punishment.reason}` : ``}` +
			...
			if (id === 'NAMELOCK' || user.namelocked) {
				...
				user.locked = punishUserid;
				user.updateIdentity();
			} else if (_optionalChain([punishmentInfo, 'optionalAccess', _68 => _68.onActivate])) {
			...
			for (const punishment of punishments) {
				if (isSharedIP) {
					...
					if (['BAN', 'LOCK', 'NAMELOCK'].includes(punishment.type)) {
						...
						_optionalChain([info, 'optionalAccess', _69 => _69.onActivate, 'optionalAccess', _70 => _70.call, 'call', _71 => _71(this, user, punishment, null, punishment.id === user.id)]);
					}
				}
			}
			if (!sharedAndHasPunishment) exports.Punishments.checkPunishmentTime(user, exports.Punishments.byWeight(punishments)[0]);
		}
	...
	checkIpBanned(connection) {
		...
		if (exports.Punishments.isSharedIp(ip)) return false;
	...
	checkNewNameInRoom(user, userid, roomid) {
		...
		if (punishments) {
			for (const punishment of punishments) {
				...
				if (_optionalChain([info, 'optionalAccess', _74 => _74.onActivate])) {
	...
	async monitorRoomPunishments(user) {
		...
		punishments = punishments.filter(([room, punishment]) => (
			_optionalChain([exports.Punishments, 'access', _84 => _84.roomPunishmentTypes, 'access', _85 => _85.get, 'call', _86 => _86(punishment.type), 'optionalAccess', _87 => _87.activatePunishMonitor])
		));
		...
		if (punishments.length >= minPunishments) {
			...
			const punishmentText = punishments.map(([room, punishment]) => {
				...
				let punishDesc = _optionalChain([exports.Punishments, 'access', _88 => _88.roomPunishmentTypes, 'access', _89 => _89.get, 'call', _90 => _90(punishType), 'optionalAccess', _91 => _91.desc]);
				if (!punishDesc) punishDesc = `punished`;
				if (punishUserid !== userid) punishDesc += ` as ${punishUserid}`;

				// Backwards compatibility for current punishments
				const trimmedReason = _optionalChain([reason, 'optionalAccess', _92 => _92.trim, 'call', _93 => _93()]);
	...
	__init19() {this.PunishmentMap = PunishmentMap}
	__init20() {this.NestedPunishmentMap = NestedPunishmentMap}
