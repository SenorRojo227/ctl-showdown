...
;
 ...
 const commands = {
	...
	async getbattlechat(target, room, user) {
		this.checkCan('lock');
		let [roomName, userName] = _lib.Utils.splitFirst(target, ',').map(f => f.trim());
		if (!roomName) {
			if (!room) {
		...
		if (roomName.startsWith(`${_configloader.Config.routes.client}/`)) {
			roomName = roomName.slice(_configloader.Config.routes.client.length + 1);
		}
		if (roomName.startsWith(`${_configloader.Config.routes.replays}/`)) {
			roomName = `battle-${roomName.slice(_configloader.Config.routes.replays.length + 1)}`;
		}
		...
		if (tarRoom) {
			...
			try {
				const raw = await _lib.Net.call(void 0, `https://${_configloader.Config.routes.replays}/${roomid.slice('battle-'.length)}.json`).get();
		...
		return this.sendReplyBox(
			_lib.Utils.html`<strong>Chat messages in the battle '${roomid}'` +
