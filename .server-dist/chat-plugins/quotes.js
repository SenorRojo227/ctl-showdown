...
convertOldQuotes();
 ...
 const commands = {
	...
	viewquote(target, room, user) {
		room = this.requireRoom();
		const roomQuotes = quotes[room.roomid];
		if (!_optionalChain([roomQuotes, 'optionalAccess', _4 => _4.length])) return this.errorReply(`This room has no quotes.`);
		const [num, showAuthor] = _lib.Utils.splitFirst(target, ',');
 ...
 const pages = {
	quotes(args, user) {
		...
		const roomQuotes = quotes[room.roomid];
		if (!_optionalChain([roomQuotes, 'optionalAccess', _5 => _5.length])) {
