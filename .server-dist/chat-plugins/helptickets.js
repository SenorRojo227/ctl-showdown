...
var _helpticketsauto = require('./helptickets-auto');
...
const TICKET_BAN_DURATION = 48 * 60 * 60 * 1000; // 48 hours
 const BATTLES_REGEX = /\bbattle-(?:[a-z0-9]+)-(?:[0-9]+)(?:-[a-z0-9]{31}pw)?/g; exports.BATTLES_REGEX = BATTLES_REGEX;
 const REPLAY_REGEX = new RegExp(
	`${_lib.Utils.escapeRegex(Config.routes.replays)}/(?:[a-z0-9]-)?(?:[a-z0-9]+)-(?:[0-9]+)(?:-[a-z0-9]{31}pw)?`, "g"
); exports.REPLAY_REGEX = REPLAY_REGEX;
...
const defaults = {responses: {}};
 ...
 const settings = (() => {
	try {
		...
		// to the JSON as well
		return {...defaults, ...JSON.parse(_lib.FS.call(void 0, SETTINGS_FILE).readSync())};
	} catch (e2) {
		return {...defaults};
	}
...
try {
		...
		if (ticket.banned) {
			...
			if (ticket.created + TICKET_CACHE_TIME <= Date.now()) {
				...
				if (ticketRoom) {
					...
					ticket.open = false;
					const startTime = _optionalChain([ticket, 'access', _ => _.state, 'optionalAccess', _2 => _2.claimTime]) || ticket.created;
 ...
 function writeSettings() {
	_lib.FS.call(void 0, SETTINGS_FILE).writeUpdate(() => JSON.stringify(exports.settings));
...
async function convertRoomPunishments() {
 ...
 class HelpTicket extends Rooms.SimpleRoomGame {
	...
	constructor(room, ticket) {
		super(room);
		this.room = room;
		this.room.settings.language = _optionalChain([Users, 'access', _3 => _3.get, 'call', _4 => _4(ticket.creator), 'optionalAccess', _5 => _5.language]) || 'english' ;
	...
	getButton() {
		...
		let details = '';
		if (_optionalChain([user, 'optionalAccess', _6 => _6.namelocked]) && !_optionalChain([this, 'access', _7 => _7.ticket, 'access', _8 => _8.state, 'optionalAccess', _9 => _9.namelocked])) {
			if (!this.ticket.state) this.ticket.state = {};
			this.ticket.state.namelocked = user.namelocked;
		}
		if (_optionalChain([this, 'access', _10 => _10.ticket, 'access', _11 => _11.state, 'optionalAccess', _12 => _12.namelocked])) {
			details += ` [${_optionalChain([this, 'access', _13 => _13.ticket, 'access', _14 => _14.state, 'optionalAccess', _15 => _15.namelocked])}]`;
		}
		if (_optionalChain([user, 'optionalAccess', _16 => _16.locked])) {
			const punishment = Punishments.userids.getByType(user.locked, 'LOCK');
			if (_optionalChain([punishment, 'optionalAccess', _17 => _17.rest, 'optionalAccess', _18 => _18.length])) {
	...
	close(result, staff) {
		...
		exports.tickets[this.ticket.userid] = this.ticket;
		writeTickets();
		this.room.modlog({action: 'TICKETCLOSE', isGlobal: false, loggedBy: _optionalChain([staff, 'optionalAccess', _19 => _19.id]) || 'unknown' });
		...
		if (!this.involvedStaff.size) {
			if (_optionalChain([staff, 'optionalAccess', _20 => _20.isStaff]) && staff.id !== this.ticket.userid) {
	...
	static list(sorter) {
		...
		return _lib.Utils.sortBy(Object.values(exports.tickets), sorter);
	}
	...
	static async getTextLogs(search, date) {
		...
		if (await _configloader.checkRipgrepAvailability.call(void 0, )) {
			const searchString = search.length > 1 ?
				...
				_lib.Utils.escapeRegex(JSON.stringify(search[1]).slice(0, -1)) :
				"";
			const args = [
	...
	static uploadReplaysFrom(text, user, conn) {
		...
		for (const roomid of rooms) {
			...
			void _optionalChain([room, 'optionalAccess', _21 => _21.uploadReplay, 'optionalCall', _22 => _22(user, conn, "forpunishment")]);
		}
	}
	...
	static formatBattleLog(logs, info, reported) {
		...
		for (const line of log) {
			...
			buf += _lib.Utils.html`${username}:</strong></span> ${message}</div>`;
		}
	...
	static getTextButton(ticket) {
		let buf = '';
		const titleBuf = [
			...ticket.text[0].split('\n').map(_lib.Utils.escapeHTML),
			...ticket.text[1].split('<br />').map(_lib.Utils.stripHTML),
		].slice(0, 3);
		...
		let namelockDisplay = '';
		if (_optionalChain([user, 'optionalAccess', _23 => _23.namelocked]) && !_optionalChain([ticket, 'access', _24 => _24.state, 'optionalAccess', _25 => _25.namelocked])) {
			if (!ticket.state) ticket.state = {};
			ticket.state.namelocked = user.namelocked;
		}
		if (_optionalChain([ticket, 'access', _26 => _26.state, 'optionalAccess', _27 => _27.namelocked])) {
	...
	static notifyResolved(user, ticket, userid = user.id) {
		...
		if (_optionalChain([result, 'optionalAccess', _28 => _28.trim, 'call', _29 => _29()])) {
			user.send(`|pm|&Staff|${user.getIdentity()}|The result was "${result}"`);
		}
		if (_optionalChain([note, 'optionalAccess', _30 => _30.trim, 'call', _31 => _31()])) {
	...
	static getTypeId(name) {
		return _optionalChain([Object, 'access', _32 => _32.entries, 'call', _33 => _33(ticketTitles), 'access', _34 => _34.find, 'call', _35 => _35(entry => entry[1] === name), 'optionalAccess', _36 => _36[0]]) || toID(name);
	}
...
function notifyUnclaimedTicket(hasAssistRequest) {
 ...
 function notifyStaff() {
	...
	for (const ticket of sortedTickets) {
		...
		const ticketRoom = Rooms.get(`help-${ticket.userid}`);
		const ticketGame = _optionalChain([ticketRoom, 'optionalAccess', _37 => _37.getGame, 'call', _38 => _38(HelpTicket)]);
		if (!ticket.claimed) {
			hasUnclaimed = true;
	...
	for (const user of Object.values(room.users)) {
		...
		for (const connection of user.connections) {
			if (_optionalChain([connection, 'access', _39 => _39.openPages, 'optionalAccess', _40 => _40.has, 'call', _41 => _41('help-tickets')])) {
...
function getReportedUser(ticket) {
	if (!_optionalChain([ticket, 'access', _42 => _42.meta, 'optionalAccess', _43 => _43.startsWith, 'call', _44 => _44('user-')])) return null;
	const id = toID(ticket.meta.slice(5));
	// ignoreit if they report themselves for w/e reason
	return (!id || id === ticket.userid) ? null : id;
}

 async function listOpponentsFrom(
	ticket
) {
	const opps = new _lib.Utils.Multiset();
	...
	return _lib.Utils.sortBy([...opps], ([, count]) => -count).map(opp => toID(opp[0]));
 ...
 async function getOpponent(link, submitter) {
	...
	if (_optionalChain([room, 'optionalAccess', _45 => _45.battle])) {
 ...
 async function getBattleLog(battle, noReplay = false) {
	...
	if (battleRoom && battleRoom.type !== 'chat') {
		...
		for (const line of battleRoom.log.log) {
			...
			if (line.startsWith('|switch|')) { // name cannot have been seen until it switches in
				...
				name = _optionalChain([name, 'optionalAccess', _46 => _46.trim, 'call', _47 => _47()]) || "";
				monTable[slot].push({
					species,
	...
	try {
		...
		if (_optionalChain([data, 'access', _48 => _48.log, 'optionalAccess', _49 => _49.length])) {
			...
			for (const line of log) {
				if (line.startsWith('|c|')) {
					...
					if (!mons[id]) mons[id] = [];
					name = _optionalChain([name, 'optionalAccess', _50 => _50.trim, 'call', _51 => _51()]) || "";
...
const cheatingScenarios = [
 ...
 const textTickets = {
	...
	inapname: {
		...
		getReviewDisplay(ticket, staff, conn, state) {
			...
			const name = _lib.Utils.escapeHTML(_optionalChain([Users, 'access', _52 => _52.getExact, 'call', _53 => _53(tar), 'optionalAccess', _54 => _54.name]) || tar);
			...
			buf += `<details ${_optionalChain([state, 'optionalAccess', _55 => _55.list]) ? 'open' : ''} class="readmore">`;
		...
		onSubmit(ticket, text) {
			if (!_optionalChain([ticket, 'access', _56 => _56.meta, 'optionalAccess', _57 => _57.startsWith, 'call', _58 => _58('user-')])) {
	...
	battleharassment: {
		...
		async getReviewDisplay(ticket, staff, connection) {
			...
			if (_optionalChain([ticket, 'access', _59 => _59.meta, 'optionalAccess', _60 => _60.startsWith, 'call', _61 => _61('room-')])) {
	...
	inappokemon: {
		...
		checker(input) {
			if (exports.BATTLES_REGEX.test(input) || exports.REPLAY_REGEX.test(input)) return true;
			return ['Please provide at least one valid battle or replay URL.'];
		},
		async getReviewDisplay(ticket, staff, conn) {
			let buf = ``;
			...
			const battleRooms = links.map(r => Rooms.get(r)).filter(room => _optionalChain([room, 'optionalAccess', _62 => _62.battle])) ;
					...
					for (const id in room.battle.playerTable) {
						...
						if (team) {
							const teamNames = team.map(p => (
								p.name !== p.species ? _lib.Utils.html`${p.name} (${p.species})` : p.species
							));
	...
	ipappeal: {
		...
		async getReviewDisplay(ticket, staff, conn, state) {
			...
			let info = _optionalChain([state, 'optionalAccess', _63 => _63.ips]);
			const stringIps = _optionalChain([info, 'optionalAccess', _64 => _64.some, 'call', _65 => _65((f) => typeof f === 'string')]);
			if (!info || stringIps) {
				const ips = stringIps ? [...info] : _optionalChain([tarUser, 'optionalAccess', _66 => _66.ips]);
				info = [];
				if (_optionalChain([ips, 'optionalAccess', _67 => _67.length])) {
					for (const ip of ips) {
			...
			for (const data of info) {
				...
				if (ipPunishments) {
					const str = ipPunishments.map(p => (
						`${_optionalChain([Punishments, 'access', _68 => _68.punishmentTypes, 'access', _69 => _69.get, 'call', _70 => _70(p.type), 'optionalAccess', _71 => _71.desc]) || p.type} as ${p.id}${p.reason ? ` (${p.reason})` : ''}`
					));
 ...
 const pages = {
	help: {
		request(query, user, connection) {
			...
			if (_optionalChain([ticket, 'optionalAccess', _72 => _72.open]) || ipTicket) {
		...
		async list(query, user, connection) {
			...
			for (const k in exports.tickets) {
				...
				if (!ticket.resolved && ticket.text && typeId === type) {
					...
					buf += await exports.textTickets[typeId].getReviewDisplay(
						ticket ,
						user,
						this.connection,
						{list: true}
		...
		tickets(query, user, connection) {
			...
			for (const ticket of sortedTickets) {
				...
				buf += `<tr><td>${icon}</td>`;
				buf += `<td>${_lib.Utils.escapeHTML(ticket.creator)}</td>`;
		...
		async logs(query, user, connection) {
			...
			_lib.Utils.sortBy(logs, log => -log.resolved.time);
 ...
 const commands = {
	report(target, room, user) {
		...
		if (this.broadcasting) {
			if (_optionalChain([room, 'optionalAccess', _73 => _73.battle])) return this.errorReply(this.tr`This command cannot be broadcast in battles.`);
	...
	appeal(target, room, user) {
		...
		const meta = this.pmTarget ? `-user-${this.pmTarget.id}` : this.room ? `-room-${this.room.roomid}` : '';
		if (this.broadcasting) {
			if (_optionalChain([room, 'optionalAccess', _74 => _74.battle])) return this.errorReply(this.tr`This command cannot be broadcast in battles.`);
	...
	helpticket: {
		...
		async submit(target, room, user, connection, cmd) {
			...
			let ticket = exports.tickets[user.id];
			const ipTicket = checkIp(user.latestIp);
			if (_optionalChain([ticket, 'optionalAccess', _75 => _75.open]) || ipTicket) {
				if (!ticket && ipTicket) ticket = ipTicket;
				if (ticket.text) {
			...
			if (textTicket) {
				...
				const validation = await _optionalChain([textTicket, 'access', _76 => _76.checker, 'optionalCall', _77 => _77(text, contextString || '', ticket.type, user, reportTarget)]);
				...
				writeTickets();
				notifyStaff();
				void _optionalChain([textTicket, 'access', _78 => _78.onSubmit, 'optionalCall', _79 => _79(ticket, [text, contextString], this.user, this.connection)]);
				void _helpticketsauto.runPunishments.call(void 0, ticket , typeId);
			...
			if (reportTargetType === 'user' && reportTarget) {
				...
				staffIntroButtons += _lib.Utils.html`<button class="button" name="send" value="/modlog room=global, user='${reportTarget}'">Global Modlog for ${reportTarget}</button> <button class="button" name="send" value="/sharedbattles ${user.id}, ${toID(reportTarget)}">Shared battles</button> `;
			}
			if (ticket.type === 'Appeal') {
				staffIntroButtons += _lib.Utils.html`<button class="button" name="send" value="/modlog room=global, user='${user.name}'">Global Modlog for ${user.name}</button>`;
			}
			...
			if (reportTargetType === 'room') {
				...
				void _optionalChain([reportRoom, 'optionalAccess', _80 => _80.uploadReplay, 'optionalCall', _81 => _81(user, connection, 'forpunishment')]);
		...
		addresponse(target, room, user) {
			this.checkCan('lock');
			const [type, name, response] = _lib.Utils.splitFirst(target, ',', 2).map(f => f.trim());
			...
			if (!(typeId in exports.textTickets)) {
				...
				return this.errorReply(`Valid types: ${Object.keys(exports.textTickets).join(', ')}.`);
			}
		...
		removeresponse(target, room, user) {
			this.checkCan('lock');
			const [type, name] = _lib.Utils.splitFirst(target, ',').map(f => f.trim());
			...
			if (!_optionalChain([exports.settings, 'access', _82 => _82.responses, 'access', _83 => _83[typeId], 'optionalAccess', _84 => _84[name]])) {
		...
		listresponses(target, room, user) {
			...
			buf += Object.keys(table).map(type => (
				...
				Object.keys(exports.settings.responses[type])
					.map(name => _lib.Utils.html`<p>- ${name}: "${exports.settings.responses[type][name]}"</p>`).join('')
			)).join('<hr />');
			return this.sendReplyBox(buf);
		},
		listresponseshelp: [
		...
		close(target, room, user) {
			if (!target) {
				if (_optionalChain([room, 'optionalAccess', _85 => _85.roomid, 'access', _86 => _86.startsWith, 'call', _87 => _87('help-')])) {
			...
			if (!_optionalChain([ticket, 'optionalAccess', _88 => _88.open]) || (ticket.userid !== user.id && !user.can('lock'))) {
		...
		async ban(target, room, user) {
			...
			if (_optionalChain([targetUser, 'optionalAccess', _89 => _89.previousIDs, 'access', _90 => _90.length])) {
			...
			for (const userObj of affected) {
				...
				if (_optionalChain([targetTicket, 'optionalAccess', _91 => _91.open])) targetTicket.open = false;
				const helpRoom = Rooms.get(`help-${userObjID}`);
				if (helpRoom) {
					const ticketGame = helpRoom.getGame(HelpTicket);
					ticketGame.writeStats('ticketban');
					helpRoom.destroy();
				} else if (_optionalChain([targetTicket, 'optionalAccess', _92 => _92.text])) {
					await HelpTicket.modlog({
						action: `TICKETBAN`,
						loggedBy: user.id,
					...
					if (staffRoom) {
						for (const curUser of Object.values(staffRoom.users)) {
							for (const conn of curUser.connections) {
								if (_optionalChain([conn, 'access', _93 => _93.openPages, 'optionalAccess', _94 => _94.has, 'call', _95 => _95(`help-text-${userObjID}`)])) {
		...
		unban(target, room, user) {
			...
			this.checkCan('lock');
			target = toID(target);
			const targetID = _optionalChain([Users, 'access', _96 => _96.get, 'call', _97 => _97(target), 'optionalAccess', _98 => _98.id]) || target ;
			...
			_optionalChain([Users, 'access', _99 => _99.get, 'call', _100 => _100(target), 'optionalAccess', _101 => _101.popup, 'call', _102 => _102(`${user.name} has ticket unbanned you.`)]);
		},
		...
		async private(target, room, user) {
			...
			const logPath = _lib.FS.call(void 0, `logs/chat/help-${userid}/${date.slice(0, -3)}/${date}.txt`);
			if (!(await logPath.exists())) {
				return this.errorReply(`There are no logs for tickets from '${userid}' on the date '${date}'.`);
			}
			if (!(await _lib.FS.call(void 0, `logs/private/${userid}`).exists())) {
				await _lib.FS.call(void 0, `logs/private/${userid}`).mkdirp();
			}
		...
		async public(target, room, user) {
			...
			const logPath = _lib.FS.call(void 0, `logs/private/${userid}/${date}.txt`);
			if (!(await logPath.exists())) {
				return this.errorReply(`There are no logs for tickets from '${userid}' on the date '${date}'.`);
			}
			const monthPath = _lib.FS.call(void 0, `logs/chat/help-${userid}/${date.slice(0, -3)}`);
 ...
 const punishmentfilter = (user, punishment) => {
	...
	if (typeof user === 'object') {
		...
		if (_optionalChain([helpRoom, 'optionalAccess', _103 => _103.game, 'optionalAccess', _104 => _104.gameid]) !== 'helpticket') return;
 ...
 const loginfilter = (user) => {
	...
	if (_optionalChain([ticket, 'optionalAccess', _105 => _105.resolved])) {
 ...
 const handlers = {
	onRoomClose(room, user, conn, isPage) {
		...
		if (_optionalChain([ticket, 'optionalAccess', _106 => _106.open]) && ticket.claimed === user.id) {
			ticket.claimed = null;
			if (_optionalChain([ticket, 'access', _107 => _107.state, 'optionalAccess', _108 => _108.claimTime])) {
