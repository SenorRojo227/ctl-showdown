 ...
 const pages = {
	proxies(query, user) {
		...
		_lib.Utils.sortBy(openProxies, ip => {
			...
			if (number === null) {
				_optionalChain([Rooms, 'access', _ => _.get, 'call', _2 => _2('upperstaff'), 'optionalAccess', _3 => _3.add, 'call', _4 => _4(`|error|Invalid IP address in IPTools.singleIPOpenProxies: '${ip}'`)]);
				return -1;
			}
			return number;
		});
		...
		html += visualizeRangeList(IPTools.ranges.filter(r => _optionalChain([r, 'access', _5 => _5.host, 'optionalAccess', _6 => _6.endsWith, 'call', _7 => _7('/proxy')])));
		html += `</table></div>`;
		return html;
	},
	...
	ranges(query, user) {
		...
		if (['all', 'mobile'].includes(type)) {
			...
			html += visualizeRangeList(IPTools.ranges.filter(range => _optionalChain([range, 'access', _8 => _8.host, 'optionalAccess', _9 => _9.endsWith, 'call', _10 => _10('/mobile')])));
			html += `</table></div>`;
		}
		if (['all', 'res', 'residential'].includes(type)) {
			html += `<div class="ladder pad"><h2>Residential IP Ranges:</h2><table>`;
			html += visualizeRangeList(IPTools.ranges.filter(range => _optionalChain([range, 'access', _11 => _11.host, 'optionalAccess', _12 => _12.endsWith, 'call', _13 => _13('/res')])));
			html += `</table></div>`;
		}
		if (['all', 'proxy', 'proxies'].includes(type)) {
			html += `<div class="ladder pad"><h2>Proxy IP Ranges:</h2><table>`;
			html += visualizeRangeList(IPTools.ranges.filter(range => _optionalChain([range, 'access', _14 => _14.host, 'optionalAccess', _15 => _15.endsWith, 'call', _16 => _16('/proxy')])));
			html += `</table></div>`;
		}
 ...
 const commands = {
	...
	nomarkshared: {
		add(target, room, user) {
			...
			if (!_optionalChain([reasonArr, 'optionalAccess', _17 => _17.length])) {
