...
const NOJOIN_COMMAND_WHITELIST = {
 ...
 let migrated = _optionalChain([global, 'access', _ => _.Chat, 'optionalAccess', _2 => _2.oldPlugins, 'access', _3 => _3['abuse-monitor'], 'optionalAccess', _4 => _4.migrated]) || false; exports.migrated = migrated;
 ...
 = (() => {
	const plugin = _optionalChain([global, 'access', _5 => _5.Chat, 'optionalAccess', _6 => _6.oldPlugins, 'access', _7 => _7['abuse-monitor']]);
	if (!_optionalChain([plugin, 'optionalAccess', _8 => _8.cache])) return {};
	if (plugin.migrated) return plugin.cache;
	for (const k in plugin.cache) {
		const cur = plugin.cache[k];
		if (typeof _optionalChain([cur, 'access', _9 => _9.recommended, 'optionalAccess', _10 => _10.type]) === 'string') { // would be object if it was the new entry
 ...
 const muted = _optionalChain([Chat, 'access', _11 => _11.oldPlugins, 'access', _12 => _12['abuse-monitor'], 'optionalAccess', _13 => _13.muted]) || new WeakMap(); exports.muted = muted;
...
const defaults = {
 ...
 const settings = (() => {
	try {
		...
		return {...defaults, ...JSON.parse(_lib.FS.call(void 0, 'config/chat-plugins/nf.json').readSync())};
	} catch (e) {
 ...
 const reviews = (() => {
	try {
		return JSON.parse(_lib.FS.call(void 0, `config/chat-plugins/artemis-reviews.json`).readSync());
	} catch (e2) {
		return {};
	}
...
function isFlaggedUserid(name, room) {
	...
	const entry = _optionalChain([exports.cache, 'access', _14 => _14[room], 'optionalAccess', _15 => _15.staffNotified]);
...
function visualizePunishmentKey(punishment, key) {
	if (key === 'secondaryTypes') {
		...
		const keys = _lib.Utils.sortBy(Object.keys(punishment.secondaryTypes));
		return `${keys.map(k => `${k}: ${punishment.secondaryTypes[k]}`).join(', ')}`;
	}
	return _optionalChain([punishment, 'access', _16 => _16[key], 'optionalAccess', _17 => _17.toString, 'call', _18 => _18()]) || "";
}
...
function displayResolved(review, justSubmitted = false) {
	...
	if (idx > -1) exports.reviews[review.staff].splice(idx, 1);
 ...
 const punishmentCache = (
	_optionalChain([Chat, 'access', _19 => _19.oldPlugins, 'access', _20 => _20['abuse-monitor'], 'optionalAccess', _21 => _21.punishmentCache]) || new WeakMap()
 ...
 const classifier = new Artemis.RemoteClassifier(); exports.classifier = classifier;
 ...
 async function runActions(user, room, message, response) {
	...
	const keys = _lib.Utils.sortBy(Object.keys(response), k => -response[k]);
	const recommended = [];
	const prevRecommend = _optionalChain([exports.cache, 'access', _22 => _22[room.roomid], 'optionalAccess', _23 => _23.recommended, 'optionalAccess', _24 => _24[user.id]]);
	for (const punishment of exports.settings.punishments) {
		if (_optionalChain([prevRecommend, 'optionalAccess', _25 => _25.type])) { // avoid making extra db queries by frontloading this check
			if (PUNISHMENTS.indexOf(punishment.punishment) <= PUNISHMENTS.indexOf(_optionalChain([prevRecommend, 'optionalAccess', _26 => _26.type]))) continue;
		}
	...
	if (recommended.length) {
		_lib.Utils.sortBy(recommended, ([punishment]) => -PUNISHMENTS.indexOf(punishment));
		...
		if (exports.cache[room.roomid]) {
			if (!exports.cache[room.roomid].recommended) exports.cache[room.roomid].recommended = {};
			exports.cache[room.roomid].recommended[user.id] = {type: punishment, reason: reason.replace(/_/g, ' ').toLowerCase()};
		}
		if (user.trusted) {
			// force just logging for any sort of punishment. requested by staff
			_optionalChain([Rooms, 'access', _27 => _27.get, 'call', _28 => _28('staff'), 'optionalAccess', _29 => _29.add, 'call', _30 => _30(
				`|c|&|/log [Artemis] <<${room.roomid}>> ${punishment} recommended for trusted user ${user.id}` +
				`${user.trusted !== user.id ? ` [${user.trusted}]` : ''} `
			), 'access', _31 => _31.update, 'call', _32 => _32()]);
			return; // we want nothing else to be executed. staff want trusted users to be reviewed manually for now
		}

		const result = await _optionalChain([punishmentHandlers, 'access', _33 => _33[_dexdata.toID.call(void 0, punishment)], 'optionalCall', _34 => _34(user, room, response, message)]);
		...
		if (result !== false) {
			...
			const notified = exports.cache[room.roomid].staffNotified;
			if (notified) {
				if (typeof notified === 'string') {
					if (notified === user.id) delete exports.cache[room.roomid].staffNotified;
				} else {
			...
			delete exports.cache[room.roomid].users[user.id]; // user has been punished, reset their counter
			// keep the cache object only if there are other users in it, since they still need to be monitored
			if (!Object.keys(exports.cache[room.roomid].users).length) {
...
function globalModlog(
	...
	void Rooms.Modlog.write(roomid || 'global', {
		...
		userid: _dexdata.toID.call(void 0, user) || undefined,
		loggedBy: 'artemis' ,
		note,
	});
}
...
function addGlobalModAction(message, room) {
	...
	_optionalChain([Rooms, 'access', _35 => _35.get, 'call', _36 => _36(`staff`), 'optionalAccess', _37 => _37.add, 'call', _38 => _38(`|c|&|/log <<${room.roomid}>> ${message}`), 'access', _39 => _39.update, 'call', _40 => _40()]);
}

function addLogButton(room) {
...
const DISCLAIMER = (
 ...
 async function lock(user, room, reason, isWeek) {
	if (exports.settings.recommendOnly) {
		_optionalChain([Rooms, 'access', _41 => _41.get, 'call', _42 => _42('staff'), 'optionalAccess', _43 => _43.add, 'call', _44 => _44(
			`|c|&|/log [Artemis] <<${room.roomid}>> ${isWeek ? "WEEK" : ""}LOCK recommended for ${user.id}`
		), 'access', _45 => _45.update, 'call', _46 => _46()]);
		addLogButton(room);
	...
	if (affected.length > 1) {
		_optionalChain([Rooms, 'access', _47 => _47.get, 'call', _48 => _48('staff'), 'optionalAccess', _49 => _49.add, 'call', _50 => _50(
			`|c|&|/log (${user.id}'s ` +
	...
	} else if (_configloader.Config.appealurl) {
		appeal += `appeal: <a href="${_configloader.Config.appealurl}">${_configloader.Config.appealurl}</a>`;
	}
...
function makeScore(roomid, result) {
	...
	for (const type in result) {
		...
		if (exports.settings.minScore && data < exports.settings.minScore) continue;
 ...
 const chatfilter = function (message, user, room) {
	...
	if (!_optionalChain([room, 'optionalAccess', _51 => _51.battle]) || !['rated', 'unrated'].includes(room.battle.challengeType)) return;
	const mutes = exports.muted.get(room);
	const muteEntry = _optionalChain([mutes, 'optionalAccess', _52 => _52.get, 'call', _53 => _53(user)]);
	if (muteEntry) {
		if (Date.now() > muteEntry) {
			...
			this.sendReply(
				...
				`<a href="https://${_configloader.Config.routes.root}/rules">Pokemon Showdown's global rules.</a> ` +
				`Repeated instances of misbehavior may incur harsher punishment.</div>`
			);
			return false;
		}
	}
	if (exports.settings.disabled) return;
	// startsWith('!') - broadcasting command, ignore it.
	if (!_configloader.Config.perspectiveKey || message.startsWith('!')) return;
	...
	void (async () => {
		...
		if (score) {
			if (!exports.cache[roomid]) exports.cache[roomid] = {users: {}};
			if (!exports.cache[roomid].users[user.id]) exports.cache[roomid].users[user.id] = 0;
			...
			if (exports.cache[roomid].users[user.id] >= calcThreshold(roomid)) {
				...
				notifyStaff();
				hitThreshold = 1;
				void _optionalChain([room, 'optionalAccess', _54 => _54.uploadReplay, 'optionalCall', _55 => _55(user, this.connection, "forpunishment")]);
			...
			await Chat.database.run(
				...
				[user.id, message, score, _lib.Utils.sortBy(flags).join(','), roomid, Date.now(), hitThreshold]
			);
		}
	})();
...
exports.chatfilter.priority = -100;
 ...
 const loginfilter = user => {
	if (_optionalChain([exports.reviews, 'access', _56 => _56[user.id], 'optionalAccess', _57 => _57.length])) {
...
function calcThreshold(roomid) {
 ...
 const handlers = {
	...
	onRoomClose(roomid, user) {
		...
		if (_optionalChain([exports.cache, 'access', _58 => _58[targetId], 'optionalAccess', _59 => _59.claimed]) === user.id) {
...
function getFlaggedRooms() {
 ...
 function writeStats(type, entry) {
	...
	try {
		_lib.FS.call(void 0, path).parentDir().mkdirpSync();
	} catch (e3) {}
	void _lib.FS.call(void 0, path).append(JSON.stringify(entry) + "\n");
} exports.writeStats = writeStats;

function saveSettings(path) {
	if (!path) path = 'nf';
	_lib.FS.call(void 0, `config/chat-plugins/${path}.json`).writeUpdate(() => JSON.stringify(exports.settings));
}

function saveReviews() {
	_lib.FS.call(void 0, `config/chat-plugins/artemis-reviews.json`).writeUpdate(() => JSON.stringify(exports.reviews));
}

 let lastLogTime = _optionalChain([Chat, 'access', _60 => _60.oldPlugins, 'access', _61 => _61['abuse-monitor'], 'optionalAccess', _62 => _62.lastLogTime]) || 0; exports.lastLogTime = lastLogTime;
...
function checkAccess(context, perm = 'bypassall') {
 ...
 const commands = {
	...
	abusemonitor: {
		...
		async compare(target) {
			checkAccess(this);
			const [base, against] = _lib.Utils.splitFirst(target, ',').map(f => f.trim());
			...
			let buf = _lib.Utils.html`<strong>Compared scores for "${base}" `;
			buf += `(<strong style="color: ${colors['1']}">1</strong>) `;
			buf += _lib.Utils.html`and "${against}" (<strong style="color: ${colors['2']}">2</strong>): </strong><br />`;
		...
		async score(target) {
			...
			const [text, scoreText] = _lib.Utils.splitFirst(target, ',').map(f => f.trim());
			...
			_optionalChain([Rooms, 'access', _63 => _63.get, 'call', _64 => _64('abuselog'), 'optionalAccess', _65 => _65.roomlog, 'call', _66 => _66(`${this.user.name} used /am score ${target}`)]);
		},
		toggle(target) {
			checkAccess(this);
			if (this.meansYes(target)) {
				if (!exports.settings.disabled) return this.errorReply(`The abuse monitor is already enabled.`);
				exports.settings.disabled = false;
			} else if (this.meansNo(target)) {
				if (exports.settings.disabled) return this.errorReply(`The abuse monitor is already disabled.`);
		...
		async resolve(target) {
			...
			let [roomid, rawResult] = _lib.Utils.splitFirst(target, ',').map(f => f.trim());
			const tarRoom = Rooms.get(roomid);
			if (!tarRoom || !exports.cache[tarRoom.roomid] || !_optionalChain([exports.cache, 'access', _67 => _67[tarRoom.roomid], 'optionalAccess', _68 => _68.staffNotified])) {
			...
			let result = _dexdata.toID.call(void 0, rawResult) === 'success' ? 1 : _dexdata.toID.call(void 0, rawResult) === 'failure' ? 0 : null;
			...
			if (_optionalChain([inserted, 'optionalAccess', _69 => _69.result]) === 1) { // (hardcode on 1 because 2 is dead)
		...
		async nojoinpunish(target, room, user) {
			this.checkCan('lock');
			const [roomid, type, rest] = _lib.Utils.splitFirst(target, ',', 2).map(f => f.trim());
			const tarRoom = Rooms.get(roomid);
			if (!tarRoom) return this.popupReply(`The room "${roomid}" does not exist.`);
			const cmd = NOJOIN_COMMAND_WHITELIST[_dexdata.toID.call(void 0, type)];
			if (!cmd) {
		...
		logs(target) {
			checkAccess(this);
			const [count, userid] = _lib.Utils.splitFirst(target, ',').map(_dexdata.toID);
			this.parse(`/join view-abusemonitor-logs-${count || '200'}${userid ? `-${userid}` : ""}`);
		},
		ul: 'userlogs',
		userlogs(target) {
			checkAccess(this, 'lock');
			return this.parse(`/join view-abusemonitor-userlogs-${_dexdata.toID.call(void 0, target)}`);
		},
		stats(target) {
			checkAccess(this);
		...
		async userclear(target, room, user) {
			...
			const targetId = _dexdata.toID.call(void 0, targetUsername);
		...
		async deletelog(target, room, user) {
			checkAccess(this);
			target = _dexdata.toID.call(void 0, target);
			if (!target) return this.parse(`/help abusemonitor`);
			const num = parseInt(target);
			if (isNaN(num)) {
		...
		editspecial(target, room, user) {
			checkAccess(this);
			if (!_dexdata.toID.call(void 0, target)) return this.parse(`/help abusemonitor`);
			let [rawType, rawPercent, rawScore] = target.split(',');
			const type = rawType.toUpperCase().replace(/\s/g, '_');
			rawScore = _dexdata.toID.call(void 0, rawScore);
			...
			const score = parseInt(rawScore) || _dexdata.toID.call(void 0, rawScore).toUpperCase() ;
			switch (typeof score) {
			case 'string':
				if (score !== 'MAXIMUM') {
			...
			if (_optionalChain([exports.settings, 'access', _70 => _70.specials, 'access', _71 => _71[type], 'optionalAccess', _72 => _72[percent]]) && !this.cmd.includes('f')) {
				return this.errorReply(`That special case already exists. Use /am forceeditspecial to change it.`);
			}
			if (!exports.settings.specials[type]) exports.settings.specials[type] = {};
		...
		deletespecial(target, room, user) {
			...
			if (!_optionalChain([exports.settings, 'access', _73 => _73.specials, 'access', _74 => _74[type], 'optionalAccess', _75 => _75[percent]])) {
		...
		addpunishment(target, room, user) {
			checkAccess(this);
			if (!_dexdata.toID.call(void 0, target)) return this.parse(`/help am`);
			const targets = target.split(',').map(f => f.trim());
			const punishment = {};
			for (const cur of targets) {
				let [key, value] = _lib.Utils.splitFirst(cur, '=').map(f => f.trim());
				key = _dexdata.toID.call(void 0, key);
				...
				case 'punishment': case 'p':
					...
					value = _dexdata.toID.call(void 0, value).toUpperCase();
				...
				case 'st': case 's': case 'secondary':
					let [sType, sValue] = _lib.Utils.splitFirst(value, '|').map(f => f.trim());
					if (!sType || !sValue) {
		...
		thresholdincrement(target, room, user) {
			checkAccess(this);
			if (!_dexdata.toID.call(void 0, target)) {
				return this.parse(`/help am`);
			}
			const [rawTurns, rawIncrement, rawMin] = _lib.Utils.splitFirst(target, ',', 2).map(_dexdata.toID);
		...
		deleteincrement(target, room, user) {
			checkAccess(this);
			if (!exports.settings.thresholdIncrement) return this.errorReply(`The threshold increment is already disabled.`);
		...
		async backupsettings(target, room, user) {
			...
			if (target) {
				...
				await _lib.FS.call(void 0, `config/chat-plugins/artemis/`).mkdirIfNonexistent();
			} else {
				target = `/nf.backup`;
			}
			saveSettings(target);
		...
		async loadbackup(target, room, user) {
			...
			const backup = await _lib.FS.call(void 0, `config/chat-plugins/${path}.json`).readIfExists();
		...
		async deletebackup(target, room, user) {
			...
			const path = _lib.FS.call(void 0, `config/chat-plugins/artemis/${target}.json`);
		...
		async backups() {
			...
			const files = await _lib.FS.call(void 0, `config/chat-plugins/artemis`).readdirIfExists();
			if (!files.length) {
		...
		async submitreview(target, room, user) {
			...
			const [roomid, reason] = _lib.Utils.splitFirst(target, ',').map(f => f.trim());
			const log = await _helptickets.getBattleLog.call(void 0, _helptickets.getBattleLinks.call(void 0, roomid)[0] || "");
			if (!log) {
				return this.popupReply(`No logs found for that roomid.`);
			}
			if (_optionalChain([exports.reviews, 'access', _76 => _76[user.id], 'optionalAccess', _77 => _77.some, 'call', _78 => _78(f => f.room === roomid)])) {
		...
		resolvereview(target, room, user) {
			checkAccess(this);
			let [userid, roomid, accurate, result] = _lib.Utils.splitFirst(target, ',', 3).map(f => f.trim());
			userid = _dexdata.toID.call(void 0, userid);
			roomid = _helptickets.getBattleLinks.call(void 0, roomid)[0] || "";
			...
			const review = exports.reviews[userid].find(f => _helptickets.getBattleLinks.call(void 0, f.room).includes(roomid));
			if (!review) {
		...
		replace(target, room, user) {
			...
			if (_dexdata.toID.call(void 0, old) === _dexdata.toID.call(void 0, newWord)) return this.errorReply(`The old word and the new word are the same.`);
			if (exports.settings.replacements[old]) {
				return this.errorReply(`The old word '${old}' is already in use (for '${exports.settings.replacements[old]}').`);
			}
	...
	abusemonitor: {
		flagged(query, user) {
			...
			for (const roomid of ids) {
				...
				buf += _lib.Utils.html`<td>${_optionalChain([Rooms, 'access', _79 => _79.get, 'call', _80 => _80(roomid), 'optionalAccess', _81 => _81.title]) || roomid}</td>`;
		...
		async view(query, user) {
			...
			if (!_dexdata.toID.call(void 0, roomid)) {
			...
			buf += _lib.Utils.html` - ${room.title}</h2>`;
			...
			const users = new _lib.Utils.Multiset();
			const logData = await _helptickets.getBattleLog.call(void 0, room.roomid, true);
			...
			for (const line of logData.log) {
				...
				const id = _dexdata.toID.call(void 0, data.user);
				if (!id) continue;
				users.add(id);
				buf += `<div class="chat chatmessage">`;
				buf += `<strong style="color: ${_helptickets.HelpTicket.colorName(id, logData)}">`;
				buf += _lib.Utils.html`<span class="username">${data.user}:</span></strong> ${data.message}</div>`;
			}
			buf += `</div></details>`;
			...
			const sortedUsers = _lib.Utils.sortBy([...users], ([id, num]) => (
				[isFlaggedUserid(id, roomid), -num, id]
			));
			for (const [id] of sortedUsers) {
				const curUser = Users.getExact(id);
				buf += _lib.Utils.html`<details class="readmore"><summary>${_optionalChain([curUser, 'optionalAccess', _82 => _82.name]) || id} `;
				...
				for (const name of punishments) {
					buf += `<form data-submitsend="/am nojoinpunish ${roomid},${_dexdata.toID.call(void 0, name)},${id},{reason}">`;
		...
		async userlogs(query, user) {
			...
			const userid = _dexdata.toID.call(void 0, query.shift());
		...
		async logs(query, user) {
			...
			_lib.Utils.sortBy(logs, log => [-log.time, log.roomid, log.userid]);
			...
			for (const log of logs) {
				const {roomid} = log;
				buf += `<tr>`;
				buf += `<td><a href="https://${_configloader.Config.routes.replays}/${roomid.slice(7)}">${roomid}</a></td>`;
				if (!userid) buf += `<td>${log.userid}</td>`;
				buf += _lib.Utils.html`<td>${log.message}</td>`;
		...
		async stats(query, user) {
			...
			const logPath = _lib.FS.call(void 0, `logs/artemis/punishments/${dateString}.jsonl`);
			...
			const reviewLogPath = _lib.FS.call(void 0, `logs/artemis/reviews/${dateString}.jsonl`);
			...
			if (punishmentStats.total) {
				...
				header = '';
				data = '';
				const sortedDayStats = _lib.Utils.sortBy(Object.keys(punishmentStats.byDay), d => new Date(d).getTime());
				...
				const sorted = _lib.Utils.sortBy(Object.entries(punishmentStats.types), e => e[1]);
			...
			for (const id of _lib.Utils.sortBy(Object.keys(staffStats), k => -staffStats[k])) {
		...
		async settings() {
			...
			if (await _lib.FS.call(void 0, 'config/chat-plugins/nf.backup.json').exists()) {
			...
			const keys = _lib.Utils.sortBy(
				Object.keys(Artemis.RemoteClassifier.ATTRIBUTES),
				k => [-Object.keys(exports.settings.specials[k] || {}).length, k]
			);
			for (const k of keys) {
		...
		reviews() {
			...
			for (const userid in exports.reviews) {
				...
				for (const review of curReviews) {
					...
					buf += `Battle: <a href="//${_configloader.Config.routes.client}/${_helptickets.getBattleLinks.call(void 0, review.room)[0]}">${review.room}</a><br />`;
					buf += _lib.Utils.html`<details class="readmore"><summary>Review details:</summary>${review.details}</details>`;
 ...
 //# sourceMappingURL=sourceMaps/abuse-monitor.js.map