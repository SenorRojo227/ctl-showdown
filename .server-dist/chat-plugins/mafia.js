...
class Mafia extends Rooms.RoomGame {
	...
	vote(userid, target) {
		...
		if (this.getHammerValue(target) <= vote.trueCount) {
			// HAMMER
			this.sendDeclare(`Hammer! ${target === 'novote' ? 'Nobody' : _lib.Utils.escapeHTML(name)} was voted out!`);
	...
	unvote(userid, force = false) {
		...
		if (!player && this.dead[userid] && this.dead[userid].restless) player = this.dead[userid];
		if (!_optionalChain([player, 'optionalAccess', _7 => _7.voting])) return this.sendUser(userid, `|error|You are not voting for anyone.`);
		if (this.voteLock && _optionalChain([player, 'optionalAccess', _8 => _8.voting])) {
	...
	voteBox() {
		...
		const list = _lib.Utils.sortBy(Object.entries(this.votes), ([key, vote]) => [
			key === plur,
			-vote.count,
		]);
		for (const [key, vote] of list) {
			buf += `${vote.count}${plur === key ? '*' : ''} ${_optionalChain([this, 'access', _9 => _9.playerTable, 'access', _10 => _10[key], 'optionalAccess', _11 => _11.safeName]) || 'No Vote'} (${vote.voters.map(a => _optionalChain([this, 'access', _12 => _12.playerTable, 'access', _13 => _13[a], 'optionalAccess', _14 => _14.safeName]) || a).join(', ')})<br />`;
		}
		return buf;
	}
	voteBoxFor(userid) {
		...
		const plur = this.getPlurality();
		for (const key of Object.keys(this.playerTable).concat((this.enableNL ? ['novote'] : [])) ) {
			if (this.votes[key]) {
			...
			if (this.votingAll && !(this.voteLock && (_optionalChain([isPlayer, 'optionalAccess', _15 => _15.voting]) || (isSpirit && this.dead[userid].voting))) &&
			(isPlayer || isSpirit)) {
	...
	applyVoteModifier(user, target, mod) {
	...
	clearHammerModifiers(user) {
		for (const player of ['novote', ...Object.keys(this.playerTable)] ) {
	...
	getPlurality() {
		...
		topVotes = _lib.Utils.sortBy(topVotes, ([key, vote]) => [
			vote.dir === 'down',
	...
	sub(player, replacement) {
		...
		if (_optionalChain([newUser, 'optionalAccess', _16 => _16.connected])) {
	...
	nextSub(userid = null) {
		...
		if (!_optionalChain([sub, 'optionalAccess', _17 => _17.connected]) || !sub.named || !this.room.users[sub.id]) return; // should never happen, just to be safe
	...
	ideaDistributeRoles(user) {
		...
		for (const p in this.playerTable) {
			...
			const u = Users.get(p);
			if (_optionalChain([u, 'optionalAccess', _18 => _18.connected])) u.send(`>${this.room.roomid}\n|notify|Pick your role in the IDEA module.`);
		}
	...
	ideaPick(user, selection) {
		...
		if (!_optionalChain([this, 'access', _19 => _19.IDEA, 'optionalAccess', _20 => _20.data])) {
	...
	ideaFinalizePicks() {
		if (!_optionalChain([this, 'access', _21 => _21.IDEA, 'optionalAccess', _22 => _22.data])) {
	...
	updateHost(...hosts) {
		...
		for (const hostid of hosts) {
			const host = Users.get(hostid);
			if (!_optionalChain([host, 'optionalAccess', _23 => _23.connected])) return;
	...
	canJoin(user, self = false, force = false) {
		if (!_optionalChain([user, 'optionalAccess', _24 => _24.connected])) return `User not found.`;
	...
	sendUser(user, message) {
		...
		if (!_optionalChain([userObject, 'optionalAccess', _25 => _25.connected])) return;
 ...
 const pages = {
	mafia(query, user) {
		...
		const game = _optionalChain([room, 'optionalAccess', _26 => _26.getGame, 'call', _27 => _27(Mafia)]);
		if (!_optionalChain([room, 'optionalAccess', _28 => _28.users, 'access', _29 => _29[user.id]]) || !game || game.ended) {
			return this.close();
		}
		...
		if (isHost) {
			...
			for (const p in game.playerTable) {
				...
				buf += game.voteModifiers[p] !== undefined ? `(votes worth ${game.getVoteValue(p )})` : '';
			...
			for (const d in game.dead) {
				...
				if (dead.treestump) buf += ` (is a Treestump)`;
				if (dead.restless) buf += ` (is a Restless Spirit)`;
				if (game.voteModifiers[d] !== undefined) buf += ` (votes worth ${game.getVoteValue(d )})`;
	...
	mafia: {
		...
		host(target, room, user, connection, cmd) {
			...
			if (nextHost) {
				...
				while ((hostid = hostQueue.shift())) {
					...
					if (!_optionalChain([targetUser, 'optionalAccess', _30 => _30.connected]) ||
			...
			if (!_optionalChain([targetUser, 'optionalAccess', _31 => _31.connected])) {
		...
		queue(target, room, user) {
			...
			case 'add':
				...
				if ((!_optionalChain([targetUser, 'optionalAccess', _32 => _32.connected])) && !command.includes('force')) {
		...
		prod(target, room, user) {
			...
			for (const player of Object.values(game.playerTable)) {
				...
				if (_optionalChain([playerid, 'optionalAccess', _33 => _33.connected]) && player.action === null) {
		...
		data(target, room, user, connection, cmd) {
			if (_optionalChain([room, 'optionalAccess', _34 => _34.settings, 'access', _35 => _35.mafiaDisabled])) return this.errorReply(`Mafia is disabled for this room.`);
			...
			// hack to let hosts broadcast
			const game = _optionalChain([room, 'optionalAccess', _36 => _36.getGame, 'call', _37 => _37(Mafia)]);
