...
function getHTMLCriteriaDescription(criteria) {
	const format = (list) => list.map(m => _lib.Utils.html`<strong>${m.name}</strong>`);
	const parts = [];
	...
	if (ability.mustHave) {
		parts.push(_lib.Utils.html`had the ability <strong>${ability.mustHave.name}</strong>`);
	}
	...
	if (item.mustHave) {
		parts.push(_lib.Utils.html`had the item <strong>${item.mustHave.name}</strong>`);
	}
	...
	if (nature.mustHave) {
		parts.push(_lib.Utils.html`had the nature <strong>${nature.mustHave.name}</strong>`);
	}
...
function setProbability(
	...
	for (let i = 0; i < rounds; i++) {
		const set = generator.randomSet(
			...
			_optionalChain([format, 'access', _ => _.ruleTable, 'optionalAccess', _2 => _2.has, 'call', _3 => _3('dynamaxclause')])
		);
...
function getLetsGoMoves(species) {
	...
	if (!_optionalChain([species, 'access', _4 => _4.randomBattleMoves, 'optionalAccess', _5 => _5.length])) return false;
...
function generateSSBMoveInfo(sigMove, dex) {
	...
	if (sigMove.shortDesc || sigMove.desc) {
		...
		if (_optionalChain([sigMove, 'access', _6 => _6.zMove, 'optionalAccess', _7 => _7.basePower])) {
			details["Z-Power"] = String(sigMove.zMove.basePower);
		} else if (_optionalChain([sigMove, 'access', _8 => _8.zMove, 'optionalAccess', _9 => _9.effect])) {
			const zEffects = {
		...
		} else if (_optionalChain([sigMove, 'access', _10 => _10.zMove, 'optionalAccess', _11 => _11.boost])) {
			details["Z-Effect"] = "";
...
function SSBSets(target) {
 ...
 const commands = {
	...
	randombattles(target, room, user) {
		...
		if (_optionalChain([room, 'optionalAccess', _12 => _12.battle, 'optionalAccess', _13 => _13.format, 'access', _14 => _14.includes, 'call', _15 => _15('nodmax')])) return this.parse(`/randombattlenodmax ${target}`);
		if (_optionalChain([room, 'optionalAccess', _16 => _16.battle, 'optionalAccess', _17 => _17.format, 'access', _18 => _18.includes, 'call', _19 => _19('doubles')])) return this.parse(`/randomdoublesbattle ${target}`);
	...
	randombattlesetprobabilities(target, room, user) {
		...
		return this.sendReplyBox(
			_lib.Utils.html`Generated ${results.rounds} sets for <strong>${species.name}</strong> in ${format.name}:<br />` +
