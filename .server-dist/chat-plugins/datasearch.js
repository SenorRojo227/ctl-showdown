...
function checkCanAll(room) {
 ...
 const commands = {
	...
	async dexsearch(target, room, user, connection, cmd, message) {
		...
		if (index >= 0) {
			const genNum = parseInt(_optionalChain([/\d*$/, 'access', _3 => _3.exec, 'call', _4 => _4(split[index]), 'optionalAccess', _5 => _5[0]]) || '');
		...
		if (!target.includes('mod=')) {
			const dex = this.extractFormat(_optionalChain([room, 'optionalAccess', _6 => _6.settings, 'access', _7 => _7.defaultFormat]) || _optionalChain([room, 'optionalAccess', _8 => _8.battle, 'optionalAccess', _9 => _9.format])).dex;
	...
	async movesearch(target, room, user, connection, cmd, message) {
		...
		const index = split.findIndex(x => /^max\s*gen/i.test(x));
		if (index >= 0) {
			const genNum = parseInt(_optionalChain([/\d*$/, 'access', _10 => _10.exec, 'call', _11 => _11(split[index]), 'optionalAccess', _12 => _12[0]]) || '');
			if (!isNaN(genNum) && !(genNum < 1 || genNum > Dex.gen)) {
				split[index] = `mod=gen${genNum}`;
				target = split.join(',');
			}
		...
		if (!target.includes('mod=')) {
			const dex = this.extractFormat(_optionalChain([room, 'optionalAccess', _13 => _13.settings, 'access', _14 => _14.defaultFormat]) || _optionalChain([room, 'optionalAccess', _15 => _15.battle, 'optionalAccess', _16 => _16.format])).dex;
			if (dex) target += `, mod=${dex.currentMod}`;
		}
	...
	for (const alts of searches) {
		...
		for (const mon in dex) {
			...
			if (alts.tiers && Object.keys(alts.tiers).length) {
				...
				if (tier.startsWith('(') && tier !== '(PU)') tier = tier.slice(1, -1) ;
				// if (tier === 'New') tier = 'OU';
			...
			for (const targetResist in alts.resists) {
				...
				if (notImmune && !move.ohko && move.damage === undefined) {
					for (const defenderType of dex[mon].types) {
						...
						const moveMod = _optionalChain([move, 'access', _17 => _17.onEffectiveness, 'optionalAccess', _18 => _18.call, 'call', _19 => _19(
			...
			for (const targetWeak in alts.weak) {
				...
				if (notImmune && !move.ohko && move.damage === undefined) {
					for (const defenderType of dex[mon].types) {
						const baseMod = mod.getEffectiveness(attackingType, defenderType);
						const moveMod = _optionalChain([move, 'access', _20 => _20.onEffectiveness, 'optionalAccess', _21 => _21.call, 'call', _22 => _22(
							{dex: mod} , baseMod, null, defenderType, move ,
						)]);
						effectiveness += typeof moveMod === 'number' ? moveMod : baseMod;
					}
		...
		for (const moveid in dex) {
			...
			for (const recoveryType in alts.other) {
				...
				if (recoveryType === "recovery") {
					...
					hasRecovery = (_optionalChain([move, 'access', _23 => _23.zMove, 'optionalAccess', _24 => _24.effect]) === 'heal');
				}
			...
			for (const boost in alts.zboost) {
				...
				if (_optionalChain([zMove, 'optionalAccess', _25 => _25.boost])) {
