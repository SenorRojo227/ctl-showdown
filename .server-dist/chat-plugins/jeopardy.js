...
const BUZZ_COOLDOWN = 500; // 0.5 seconds
 ...
 class Jeopardy extends Rooms.RoomGame {
	...
	constructor(room, user, categoryCount, questionCount, playerCap) {
		super(room);
	...
	getGrid() {
		...
		for (const userID in this.playerTable) {
			const player = this.playerTable[userID];
			const bold = _optionalChain([this, 'access', _ => _.curPlayer, 'optionalAccess', _2 => _2.name]) === player.name;
			buffer += `${bold ? "<b>" : ""}<font size=2>${_lib.Utils.escapeHTML(player.name)} (${(player.points || 0)})${bold ? "</b>" : ""}<br />`;
		}
	...
	wager(amount, user) {
		if (this.state !== "wagering" && (!this.finals || _optionalChain([this, 'access', _3 => _3.curPlayer, 'optionalAccess', _4 => _4.id]) !== user.id)) {
	...
	doFinals() {
		if (!this.playerTable) return _optionalChain([this, 'access', _5 => _5.room, 'optionalAccess', _6 => _6.add, 'call', _7 => _7(`Could not play finals because the player table does not exist.`)]);
...
class JeopardyGamePlayer extends Rooms.RoomGamePlayer {
 ...
 //# sourceMappingURL=sourceMaps/jeopardy.js.map