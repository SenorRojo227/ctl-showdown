...
const LOGIN_NOTIFY_THROTTLE = 15 * 60 * 1000;
 ...
 const Friends = new class {
	...
	async notifyConnection(user) {
		...
		const message = `/nonotify Your friend <username class="username">${_utils.Utils.escapeHTML(user.name)}</username> has just connected!`;
		...
		case 'spectate':
			...
			for (const friend of friends) {
				const curBattles = [...friend.inRooms]
					.filter(id => {
						const battle = _optionalChain([Rooms, 'access', _16 => _16.get, 'call', _17 => _17(id), 'optionalAccess', _18 => _18.battle]);
						return (
