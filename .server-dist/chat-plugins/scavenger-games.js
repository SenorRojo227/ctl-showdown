...
const TWISTS = {
	...
	spamfilter: {
		...
		onConfirmCompletion(player, time, blitz, place, result) {
			blitz = result.blitz;
			time = result.time;
			const deductionMessage = _optionalChain([player, 'access', _9 => _9.incorrect, 'optionalAccess', _10 => _10.length]) ?
				Chat.count(player.incorrect, 'incorrect guesses', 'incorrect guess') :
				"Perfect!";
			return `<em>${_lib.Utils.escapeHTML(player.name)}</em> has finished the hunt! (Final Time: ${time} - ${deductionMessage}${(blitz ? " - BLITZ" : "")})`;
		},

		onEnd() {
			_lib.Utils.sortBy(this.completed, entry => entry.total);
		},
	},
	...
	blindincognito: {
		...
		onPreComplete(player) {
			...
			const blitz = now - this.startTime <= 60000 && canBlitz &&
				(_optionalChain([this, 'access', _11 => _11.room, 'access', _12 => _12.settings, 'access', _13 => _13.scavSettings, 'optionalAccess', _14 => _14.blitzPoints, 'optionalAccess', _15 => _15[this.gameType]]) || this.gameType === 'official');
...
const MODES = {
	...
	teamscavs: {
		...
		teamAnnounce(player, message) {
			...
			for (const userid of team.players) {
				...
				if (!_optionalChain([user, 'optionalAccess', _16 => _16.connected])) continue; // user is offline
 ...
 class ScavengerGameTemplate {
	...
	eliminate(userid) {
		if (!_optionalChain([this, 'access', _17 => _17.playerlist, 'optionalAccess', _18 => _18.includes, 'call', _19 => _19(userid)])) return false;
...
const LoadGame = function (room, gameid) {
 ...
 //# sourceMappingURL=sourceMaps/scavenger-games.js.map