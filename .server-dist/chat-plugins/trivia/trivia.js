...
var _database = require('./.data-distbase');
...
const MAX_ANSWER_LENGTH = 32;
 ...
 const database = new (0, _database.TriviaSQLiteDatabase)('config/chat-plugins/triviadata.json'); exports.database = database;
...
async function getQuestions(
	...
	if (categories === 'random') {
		...
		const lastCategoryID = toID(_optionalChain([history, 'optionalAccess', _ => _[0], 'access', _2 => _2.category])).replace("random", "");
		const randCategory = _lib.Utils.randomElement(
		...
		return exports.database.getQuestions('all', limit, {order});
	} else {
		for (const category of categories) {
			if (!ALL_CATEGORIES[category]) throw new Chat.ErrorMessage(`"${category}" is an invalid category.`);
		}
		return exports.database.getQuestions(categories, limit, {order});
	}
}
...
class Ladder {
	...
	async computeCachedLadder() {
		...
		for (const [lb, data] of Object.entries(leaderboards) ) {
			...
			for (const key of ['score', 'totalPoints', 'totalCorrectAnswers'] ) {
				_lib.Utils.sortBy(leaders, ([, scores]) => -scores[key]);
 ...
 const cachedLadder = new Ladder(); exports.cachedLadder = cachedLadder;
...
class TriviaPlayer extends Rooms.RoomGamePlayer {
 ...
 class Trivia extends Rooms.RoomGame {
	...
	onConnect(user) {
		...
		if (!_optionalChain([player, 'optionalAccess', _3 => _3.isAbsent])) return false;
	...
	getPrizes() {
		...
		return (_optionalChain([LENGTHS, 'access', _4 => _4[this.game.length], 'optionalAccess', _5 => _5.prizes]) || [5, 3, 1]).map(prize => prize * multiplier);
	}
 ...
 class Mastermind extends Rooms.SimpleRoomGame {
	...
	formatPlayerList() {
		return _lib.Utils.sortBy(
			...
			return `${name} (${_optionalChain([this, 'access', _6 => _6.leaderboard, 'access', _7 => _7.get, 'call', _8 => _8(player.id), 'optionalAccess', _9 => _9.score]) || "0"})`;
		}).join(', ');
	}
	...
	startRound(playerID, category, questions, timeout) {
		...
		setTimeout((id) => {
			...
			const points = _optionalChain([this, 'access', _10 => _10.currentRound, 'access', _11 => _11.playerTable, 'access', _12 => _12[playerID], 'optionalAccess', _13 => _13.points]);
	...
	kick(toKick, kicker) {
		...
		if (_optionalChain([this, 'access', _14 => _14.currentRound, 'optionalAccess', _15 => _15.playerTable, 'access', _16 => _16[toKick.id]])) {
...
const triviaCommands = {
	...
	async new(target, room, user, connection, cmd) {
		...
		if (isRandomMode) {
			...
			mode = _lib.Utils.shuffle(acceptableModes)[0];
		}
	...
	async accept(target, room, user, connection, cmd) {
		...
		if (toID(target) === 'all') {
			if (isAccepting) await exports.database.addQuestions(submissions);
	...
	async delete(target, room, user) {
		...
		const question = _lib.Utils.escapeHTML(target).trim();
		if (!question) {
	...
	async edit(target, room, user) {
		...
		const {category} = await exports.database.ensureQuestionExists(_lib.Utils.escapeHTML(oldQuestionText));
		...
		if (!isQuestionOnly) {
			...
			if (!answerArgs) {
				...
				for (const arg of _optionalChain([answerArgs, 'optionalAccess', _19 => _19.split, 'call', _20 => _20(',')]) || []) {
		...
		await exports.database.editQuestion(
			_lib.Utils.escapeHTML(oldQuestionText),
			// Cast is safe because if `newQuestionText` is undefined, `isAnswersOnly` is true.
			isAnswersOnly ? undefined : _lib.Utils.escapeHTML(newQuestionText),
	...
	async rank(target, room, user) {
		...
		const score = (
			await exports.database.getLeaderboardEntry(userid, 'nonAlltime') ||
			{score: 0, totalPoints: 0, totalCorrectAnswers: 0}
		);
		const cycleScore = await exports.database.getLeaderboardEntry(userid, 'cycle');

		const ranks = await _asyncOptionalChain([(await exports.cachedLadder.get('nonAlltime')), 'optionalAccess', async _21 => _21.ranks, 'access', async _22 => _22[userid]]);
		const allTimeRanks = await _asyncOptionalChain([(await exports.cachedLadder.get('alltime')), 'optionalAccess', async _23 => _23.ranks, 'access', async _24 => _24[userid]]);
		const cycleRanks = await _asyncOptionalChain([(await exports.cachedLadder.get('cycle')), 'optionalAccess', async _25 => _25.ranks, 'access', async _26 => _26[userid]]);
		...
		function display(
			...
			if (!_optionalChain([scores, 'optionalAccess', _27 => _27[key]])) return `<td>N/A</td>`;
			return _lib.Utils.html`<td>${scores[key]}${_optionalChain([ranking, 'optionalAccess', _28 => _28[key]]) ? ` (rank ${ranking[key]})` : ``}</td>`;
		}
	...
	async ladder(target, room, user, connection, cmd) {
		...
		const ladder = await _asyncOptionalChain([(await exports.cachedLadder.get(leaderboard)), 'optionalAccess', async _29 => _29.ladder]);
		if (!_optionalChain([ladder, 'optionalAccess', _30 => _30.length])) return this.errorReply(this.tr`No Trivia games have been played yet.`);
	...
	async removeleaderboardentry(target, room, user) {
		...
		await Promise.all(
			(Object.keys(_database.LEADERBOARD_ENUM) )
	...
	triviahelp() {
		this.sendReply(
			...
			`</ul></details>` +
				...
				_lib.Utils.html`<li><code>${'/trivia edit <old question text> | <new question text> | <answer1, answer2, ...>'}</code>: Edit a question in the trivia database, replacing answers if specified. Requires: % @ # &` +
				_lib.Utils.html`<li><code>${'/trivia edit answers | <old question text> | <new answers>'}</code>: Replaces the answers of a question. Requires: % @ # &</li>` +
				_lib.Utils.html`<li><code>${'/trivia edit question | <old question text> | <new question text>'}</code>: Edits only the text of a question. Requires: % @ # &</li>` +
