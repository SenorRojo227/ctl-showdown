 ...
 */
 class RuleTable extends Map {
	...
	hasComplexBans() {
		return (_optionalChain([this, 'access', _ => _.complexBans, 'optionalAccess', _2 => _2.length]) > 0) || (_optionalChain([this, 'access', _3 => _3.complexTeamBans, 'optionalAccess', _4 => _4.length]) > 0);
	}
	...
	getRuleTable(format, depth = 1, repeals) {
		...
		for (const rule of ruleset) {
			...
			for (const [ruleid, sourceFormat] of subRuleTable) {
				...
				if (!_optionalChain([repeals, 'optionalAccess', _7 => _7.has, 'call', _8 => _8(ruleid)])) {
		...
		for (const rule of ruleTable.keys()) {
			...
			if (subFormat.exists) {
				const value = _optionalChain([subFormat, 'access', _9 => _9.onValidateRule, 'optionalAccess', _10 => _10.call, 'call', _11 => _11({format, ruleTable, dex: this.dex}, ruleTable.valueRules.get(rule ))]);
		...
		for (const matchType of matchTypes) {
			...
			if (table.hasOwnProperty(id)) {
				if (matchType === 'pokemon') {
					...
					if ((species.otherFormes || species.cosmeticFormes) && ruleid !== species.id + _dexdata.toID.call(void 0, species.baseForme)) {
