...
var _lib = require('../.lib-dist');
 ...
 class Battle {
	...
	nextTurn() {
		...
		const dynamaxEnding = [];
		for (const pokemon of this.getAllActive()) {
			if (_optionalChain([pokemon, 'access', _10 => _10.volatiles, 'access', _11 => _11['dynamax'], 'optionalAccess', _12 => _12.turns]) === 3) {
	...
	boost(
		...
		for (boostName in boost) {
			...
			if (boostBy) {
				success = true;
				switch (_optionalChain([effect, 'optionalAccess', _14 => _14.id])) {
				case 'bellydrum':
	...
	directDamage(damage, target, source = null, effect = null) {
		...
		if (!_optionalChain([target, 'optionalAccess', _15 => _15.hp])) return 0;
		if (!damage) return 0;
		...
		if (this.gen <= 1 && this.dex.currentMod !== 'gen1stadium' &&
			...
			if (_optionalChain([source, 'optionalAccess', _16 => _16.volatiles, 'access', _17 => _17['substitute']])) {
	...
	heal(damage, target, source = null, effect = null) {
		...
		if (!_optionalChain([target, 'optionalAccess', _18 => _18.hp])) return false;
		if (!target.isActive) return false;
		if (target.hp >= target.maxhp) return false;
		const finalDamage = target.heal(damage, source, effect);
		switch (_optionalChain([effect, 'optionalAccess', _19 => _19.id])) {
		case 'leechseed':
		case 'rest':
	...
	getTarget(pokemon, move, targetLoc, originalTarget) {
		...
		if (move.target !== 'randomNormal' && this.validTargetLoc(targetLoc, pokemon, move.target)) {
			...
			if (_optionalChain([target, 'optionalAccess', _20 => _20.fainted])) {
				if (this.gameType === 'freeforall') {
	...
	runAction(action) {
		const pokemonOriginalHP = _optionalChain([action, 'access', _21 => _21.pokemon, 'optionalAccess', _22 => _22.hp]);
		let residualPokemon = [];
		...
		if (!this.queue.peek() || (this.gen <= 3 && ['move', 'residual'].includes(this.queue.peek().choice))) {
			...
			return false;
		} else if (_optionalChain([this, 'access', _23 => _23.queue, 'access', _24 => _24.peek, 'call', _25 => _25(), 'optionalAccess', _26 => _26.choice]) === 'instaswitch') {
			return false;
		}
		...
		if (this.gen >= 8 && (_optionalChain([this, 'access', _27 => _27.queue, 'access', _28 => _28.peek, 'call', _29 => _29(), 'optionalAccess', _30 => _30.choice]) === 'move' || _optionalChain([this, 'access', _31 => _31.queue, 'access', _32 => _32.peek, 'call', _33 => _33(), 'optionalAccess', _34 => _34.choice]) === 'runDynamax')) {
