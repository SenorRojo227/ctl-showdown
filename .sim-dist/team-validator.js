 ...
 */
 ...
 = {}
		...
		if (format.team) {
			...
			try {
				const testTeamGenerator = _teams.Teams.getGenerator(format, testTeamSeed);
	...
	getEventOnlyData(species, noRecurse) {
		...
		if (!_optionalChain([learnset, 'optionalAccess', _ => _.eventOnly])) {
	...
	validateSet(set, teamHas) {
		...
		if (!_optionalChain([set, 'access', _2 => _2.moves, 'optionalAccess', _3 => _3.length])) {
	...
	validateSource(
		...
		if (source.charAt(1) === 'S') {
			...
			eventData = _optionalChain([eventLsetData, 'access', _4 => _4.eventData, 'optionalAccess', _5 => _5[parseInt(splitSource[0])]]);
			if (!eventData) {
	...
	validateForme(set) {
		...
		if (species.name in crowned) {
			const behemothMove = set.moves.map(_dex.toID).indexOf(crowned[species.name] );
			if (behemothMove >= 0) {
		...
		if (this.format.id.startsWith('gen8pokebilities')) {
			...
			if (ability.id !== this.toID(species.abilities['S'])) {
				for (const abilityName of unSeenAbilities) {
					setHas['ability:' + _dex.toID.call(void 0, abilityName)] = true;
				}
			}
		}
	...
	checkCanLearn(
		...
		while (_optionalChain([species, 'optionalAccess', _6 => _6.name]) && !alreadyChecked[species.id]) {
